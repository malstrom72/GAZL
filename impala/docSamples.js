...
const SAMPLES = [
	...
	{ name: "a subscript names every axis or none", expect: "E206",
		src: "global int array board[3, 4]\n"
				+ "export function main() locals int v { v = global board[1]; }\n" },
	{ name: "a shaped array needs one brace group per axis", expect: "E422",
		src: "global int array board[2, 3] = { 1, 2, 3, 4, 5, 6 }\nexport function main() { }\n" },
	{ name: "a host-owned array states its rank", expect: "E432",
		src: "extern struct H { int a; int array cells }\nexport function main() { }\n" },
	...
	/* `inline function` is NOT in this list on the GAZL 2 line. E439 is the Impala 2 branch's refusal,
	   which exists because that branch must run on a GAZL 1.0 engine with no `SCOP`/`ENDS` for an
	   expansion to declare its locals in. Here the instructions exist and the feature is live, so the
	   sample would compile clean - which is what this gate caught when the two branches met. */

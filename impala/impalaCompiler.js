	...
	: undefined;
{
    ...
    checkIndexUse = function (expr) {
        ...
        /* NOT emitted here - see flushIndexGuards. NUMERIC indices for one array collapse to the largest,
           which implies every smaller one because the stride is positive; a SYMBOLIC index cannot be
           ordered, so its own text goes in the key and each distinct one is kept. */
        var key = op.ext.sym + (typeof op.k === 'number' ? '' : '|' + op.k);
    ...
    flushIndexGuards = function () {
        for (var key in pendingGuards) {
            ...
            var lhs = '#' + op.k;
            var tests = [];
            if (typeof op.k !== 'number') {                       /* a SYMBOLIC index: the lower bound is
                                                                     not knowable here either, so ask for
                                                                     it in the same assertion. Inverted,
                                                                     and BEFORE any scaling - a positive
                                                                     stride preserves the sign. */
                tests.push(['LSS?', lhs + ' #0']);
            }
            ...
            tests.push(['LSS?', lhs + ' #' + op.ext.sym]);
            assembleAssert(tests,

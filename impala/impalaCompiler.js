	...
	: undefined;
{
    ...
    emitStructLayout = function (name) {
        ...
        for (var i = 0; i < s.fields.length; ++i) {
            ...
            if (f.type === 'S') {                                 /* nested by-value struct */
                ...
                output(fieldExtentSymbol(name, f.name) + ':' + T + '! DEFi #'
                        + (isStructAtom(f.elem) ? '<t>' : f.size));
                output(T + '! ADDi <a> #<a> #' + fieldExtentSymbol(name, f.name));
    ...
    /* The extent of a struct ARRAY FIELD, in words. `.x.` cannot serve: it is `.x.<owner>.<name>` over
       FUNCTIONS, and a struct may share a function's name (`struct Foo` beside `function Foo`), so a
       field `v` and a local `v` would mint one symbol twice. `.z.` is struct-only and already means
       "size in words", so a third part names a FIELD's size - `.o.S.v` and `.z.S.v` are the offset and
       size of the same field. Minted only for array fields: a scalar is 1 and a by-value field is
       `.z.Inner`, so those already have a name and need no second one. */
    fieldExtentSymbol = function (structName, fieldName) {
        return '.z.' + structName + '.' + fieldName;
    };

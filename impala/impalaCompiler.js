	...
	: undefined;
{
     ...
     */
    /* What a subscript needs to know about the array it indexes: `n` decides the two STATIC tiers and is
       a number only when the source wrote one, `sym` is the `.z.` extent in words and is what the
       RUNTIME check compares against, so it works for a symbolic extent too. A declared array records
       `extentSym` at its declaration - the one point where a local's owning function is in scope. */
    arrayExtent = function (sym, name) {
        return (sym === undefined || sym.extentSym === undefined) ? undefined
                : { n: sym.size, sym: sym.extentSym, what: name };
    };
    ...
    checkOnePast = function (expr) {
        ...
        expr.onePast = undefined;
        fail('Index ' + op.n + ' is outside the extent ' + op.n + ' of this array',
                op.src, op.off, 'E461',
                'one past the end is an address only - write `&` to take it, or index 0 to ' + (op.n - 1));
    };
    ...
    makeRValue = function (expr, classes) {
        ...
        expr = metaSlot(expr);
        checkOnePast(expr);                              /* reading it - reference() would have cleared the flag */
    ...
    setPlace = function (rec, baseKind, base, offParts, structName, root, arrayOf, dynIndex, extent) {
        ...
        slot.extent   = extent;                                   /* declared element COUNT, set by an array FIELD and
                                                                     nowhere else - that is the only overrun GAZL cannot
                                                                     see, since it stays inside the struct's allocation.
                                                                     A plain array (local or global) has decayed to a
                                                                     pointer before it is subscripted, and an array of
                                                                     structs is caught as "Offset out of bounds", so
                                                                     neither is worth re-checking here. Every other
                                                                     setPlace passes nothing, so a pooled slot cannot
                                                                     inherit a stale extent. */
        ...
        slot.onePast  = undefined;                                /* pooled slot: never inherit another subscript's end-pointer flag */
    };
    ...
    subscriptStruct = function (x, idx, sourceCode, sourceOffset) {
        ...
        /* A struct array FIELD is the one overrun nothing else sees: GAZL bounds-checks the whole
           allocation, which `s.v[5]` on `int array v[2]` never leaves - it just lands in the next field.
           Decidable only when both the index and the extent are numbers here; a symbolic extent (`v[SN]`)
           or a runtime index is not knowable at Impala compile time. The index test accepts a MINUS sign
        ...
        if (extent !== undefined && /^[0-9]+$/.test('' + extent.n) && /^#-?[0-9]+$/.test(idxRV)) {
            ...
            /* ONE PAST THE END is an address, not an element: `&s.v[2]` on `v[2]` is the standard end
               ...
               the flag; makeRValue and assign fail on anything that still carries one, which is exactly
               the set of uses that DEREFERENCE the element. Two past is wild in every context. */
            if (ck < 0 || ck > cn) {
                fail('Index ' + ck + ' is outside the extent ' + cn + ' of this array',
                        sourceCode, sourceOffset, 'E461',
                        'valid indices are 0 to ' + (cn - 1));
            }
            if (ck === cn) onePast = { n: cn, src: sourceCode, off: sourceOffset };
        }
        ...
        if (/^#[0-9]+$/.test(idxRV)) {                           /* constant index -> fold into the compile-time offset parts */
            ...
            metaSlot(x).onePast = onePast;                       /* set AFTER the terminal call - both clear the slot */
            return;
        }
    ...
    fieldAccess = function (x, fieldName, arrow, sourceCode, sourceOffset) {
        ...
        checkOnePast(x);                                 /* `e[[2]].a` reaches INTO the element past the end */
    ...
    binaryOp = function (operator, leftx, rightx,
        ...
        if (operator === '=[]') {
            ...
            if (leftx.operator === ':=' && op1[0] === '&') {
                ...
                if (op2[0] !== '#') emitRangeCheck(op2, xt, sourceCode, sourceOffset);
                ...
                if (op2b[0] !== '#') emitRangeCheck(op2b, xt, sourceCode, sourceOffset);
    ...
    assign = function (x, leftx, rightx,
        ...
        leftx  = metaSlot(leftx);
        rightx = metaSlot(rightx);
        checkOnePast(leftx);                             /* writing it: a target is never an address */
        checkOnePast(rightx);                            /* `x = s.v[2]` reuses the operand without makeRValue */
    ...
    reference = function (operator, expr, sourceCode, sourceOffset) {
        ...
        expr = metaSlot(expr);
        expr.onePast = undefined;                    /* taking the ADDRESS of one past the end is the whole
                                                        point of allowing it - see subscriptStruct. Cleared
                                                        before the `=[]$` branch below calls makeRValue. */
        ...
        if (!isGlobal && (p = sym.locals['$' + name])) {
            ...
            if (p.type === 'A' && isStructAtom(p.elem)) {   /* struct-element array -> a foldable local array place (base:offset, no ADRL) */
                setPlace(x, 'local', '$' + name, [], undefined, p.elem, p.elem,
                        undefined, arrayExtent(p, name));
                return;
            }
            if (p.type === 'A') {
                makeMeta(x, '=&', 'p', undefined,
                                  '$' + name, '*0');
                metaSlot(x).extent = arrayExtent(p, name);   /* survives until the next makeMeta */
            } else {
                makeMeta(x,
                                  (p.readonly ? ':=' : '='),
                                  p.type,

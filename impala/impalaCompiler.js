	...
	: undefined;
{
    ...
    var pendingGuards = [];                              /// deferred index assertions found mid-expression, emitted at the end of the function (flushIndexGuards)
    ...
    emitRangeCheck = function (idxOp, extent, sourceCode, sourceOffset) {
        if (!rangeChecks || extent === undefined || idxOp.charAt(0) === '#') {
            return;                                               /* a CONSTANT index is checkConstIndex's,
                                                                     decided without emitting anything */
        }
    ...
    endDebugGuard = function (ok, msg, text, sourceCode, sourceOffset) {
    ...
    checkIndexUse = function (expr) {
        ...
        pendingGuards.push(op);                          /* NOT emitted here - see flushIndexGuards */
    };

    /* The deferred guards a function body accumulated, emitted once its own code is out. They cannot go
       out where they are FOUND: `declare` flushes pending metacode first, and a subscript is discovered
       halfway through building an expression, so flushing there renders a half-built meta and throws.
       Position is free anyway - the guard executes nothing, and every name it uses (`.z.<path>`, a
    ...
    flushIndexGuards = function () {
        for (var i = 0; i < pendingGuards.length; ++i) {
            ...
            declare('!', 'globals', skip, undefined, true, undefined, op.src, op.off);
        }

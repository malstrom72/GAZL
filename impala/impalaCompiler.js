	...
	: undefined;
{
    ...
    var pendingGuards = {};                              /// deferred index assertions found mid-expression, keyed by extent symbol, emitted at the end of the function (flushIndexGuards)
    ...
    /* `--range-checks`: the DEBUG-gated RUNTIME bounds test for a subscript whose index is not a
       ...
       three ALU instructions to save one branch, six runtime instructions against four, and it computes
       `extent - 1` at RUN time from a constant the assembler already knows. Two compares also read as
       what they are.
    ...
    checkIndexUse = function (expr) {
        ...
        /* NOT emitted here - see flushIndexGuards. Keyed by extent symbol and keeping the LARGEST index,
    ...
    assembleAssert = function (cmp, operands, message, sourceCode, sourceOffset) {
        var skip = '.g' + (guardCounter++);
        declare('! ' + cmp, 'globals', undefined, 'i', true,
                operands + ' @' + skip, sourceCode, sourceOffset);
        declare('! FAIL ' + message, 'globals', undefined, undefined, true, undefined,
                sourceCode, sourceOffset);
        ...
        for (var sym in pendingGuards) {
            ...
            if (op.ext.stride !== undefined) {
                /* Struct elements: `.z.` counts WORDS, the index counts ELEMENTS, so scale first. `<g>`
                   ...
                   function's scratch window has closed, and borrowing here tripped the pool's own leak
                   assert for every function but the last. One name serves every guard because the
                   `! MULi` is consumed by the `! LSSi` on the very next line. The whole instruction goes
                   in the MNEMONIC, like the `! FAIL` below: declare() hands its VALUE to returnBack,
                   which would see a leading `<` and push this line into the scratch stock. */
                declare('! MULi <g> #' + op.k + ' #' + op.ext.stride,

...
{
    /**
     * emitLine(line) - every GAZL line leaves through here.
     *
     * A function's frame declarations are emitted as its head is parsed, but an inline expansion
     * happens mid-body and needs to ADD declarations to that head (GAZL requires them before the
     * first real instruction). So while a function body is being parsed, head lines are buffered in
     * `headSink` and replayed once the body - and therefore every expansion in it - is
     * known. Outside a function the sink is null and lines go straight out, unchanged.
     */
    ...
    var gazl2          = (typeof _hostOptions !== 'undefined' && _hostOptions != null
            && !!_hostOptions.gazl2);                           /// `--gazl2` targets the GAZL 2 assembler: struct initializers
                                                                /// are PLACED with SEEK regions instead of positional DATA,
                                                                /// which lifts E454 and E459. Default output is byte-identical.
    ...
    var META_TO_GAZL   = {};
    var SUPPORTED_OPS  = {};
    var inlineLocals   = [];                            /// scalar locals of the inline function being parsed
    var inlineMode     = false;                         /// true while parsing an `inline function` body - emit nothing
    var INLINE_PURE    = {};                          /// operators whose write destination the compiler can NAME
    var inlineCounter  = 0;                           /// per-expansion suffix, keeps inlined labels unique
    var headSink       = null;                        /// while a function body is parsed, its head lines buffer here (see emitLine)
    var tailFn         = undefined;                   /// name of the function being parsed - the only target `tail` accepts
    var inlineAddrAt   = {};                          /// inline fn name -> where it was last used as a VALUE, for E435's caret
    var CASTS_TO_TYPES = {};
    var ZEROES = {};
    var TYPE_SUFFIXES  = {};
    var VERBOSE_TYPES  = {};
    ...
    map(META_TO_GAZL,
        ...
        '-->','GOTO', '-->#','SWCH',  '...','FOR?',  '()','CALL', 'tail()','TAIL', '--^','RETU', 'FAIL','FAIL',
        '|','IOR?', '&','AND?', '^','XOR?', '<<','SHL?', '>>>','SHRu', '>>','SHR?',
        '+','ADD?', '-','SUB?', '*','MUL?', '/','DIV?', '%','MOD?', 'd','DIFp', 'dt','DIFt',
    ...
    emitFunctionSignature = function (name, sourceCode, sourceOffset) {
        ...
        /* An `inline function` has no out-of-line copy, so it publishes neither a FUNC label nor a
           signature row - there is nothing for a caller or the validator to link against. (Restored
           after the Impala 2 merge, which took that branch's version of this function wholesale; without
           it every inline function emitted a header with no body, and the assembler said "Missing RETU".) */
        if (inlineMode) {
            entry.kind = 'FUNC';                          /* still claims the name (duplicate detection) */
            return;
        }
    ...
    function emitStandaloneSignatureComment(comment) {
        if (comment) {
            emitLine('; ' + comment);                         /* emitLine itself no-ops without an output */
        }
    }
    ...
    processBranches = function () {
        ...
        for (var i = metacode.length - 1; i >= 0; --i) {
            ...
            /* An inline expansion pastes records this pass ALREADY ran over, in the callee. Running it
               again shifts a comparison's operands left a second time, dropping operand 0 and writing a
               null target - `GRTi %1 #0 @ok` became `GRTi #0 @ok`. It bit only the comparisons that
               survive the first pass unrenamed: one targeting FALSE is rewritten to `!<` and no longer
               matches the case below, which is why `while` looked fine and `assert` did not. A pasted
               record also breaks any branch chain being threaded across it, so reset the state too. */
            if (inst.settled) {                 /* already resolved when its inline body was captured */
        ...
        for (i = 0; i < metacode.length; ++i) {
            ...
            for (var n = i + 1; n <= metacode.length; ++n) {
                if (n === metacode.length) {
                    /* Off the end. A function RETURNS here, so a goto landing on this label is a
                       return - but an `inline` body does NOT: it is pasted mid-caller and control
                       runs on into it. Rewriting the goto there returned from the CALLER, which
                       silently truncated the caller at the first expansion that took such an arm. */
                    leadsTo[metacode[i].operands[0]] = !inlineMode;
                    break;
                }
                var nx = metacode[n];
    ...
    /* The `<X>` a COMPOUND operand ends with (`*<A>`, `#<D>`, `&g:<D>`, `%<A>`), or null. The one
       spelling of that grammar - returnBack and expandInline's operand mapper both read it. */
    trailingScratch = function (op) {
        return (op.length > 3 && op.charAt(op.length - 1) === '>' && op.charAt(op.length - 3) === '<'
                ? op.substr(op.length - 3) : null);
    };
    ...
    returnBack = function (op) {
        ...
        var trailing = trailingScratch(op);
        if (trailing !== null) {
            returnBack(trailing);
        }
    ...
    /* --------------------------------------------------------- *
     *  `inline function` expansion (see design/impala/Inlining.md)        *
     * --------------------------------------------------------- */

    /* Operators whose destination the compiler can NAME, so they cannot disturb a caller's local.
       A POSITIVE list on purpose: an operator nobody classified defaults to opaque, so a write form
       added later fails safe instead of silently making substitution unsound. */
    map(INLINE_PURE,
        '=[]',1, '=[]$',1, '=*',1, ':=*',1, '=&',1, '[]$=',1,
        'd',1, 'dt',1, '-->',1, '-->#',1, '...',1, '<--',1
    );
    /* ...plus every INLINE_IMM_OK operator (seeded below its own definition): computing into a named
       destination is what makes an operator both immediate-friendly and pure, so one list serves both
       and the two cannot drift. */

    /* Which operand positions accept an immediate (`#const`), plus a leading `d` when position 0 is the
       DESTINATION. A positive list like INLINE_PURE: an operator missing here forces the argument to be
       materialised, which is always correct. Verified one by one against the operand forms in
       docs/gazl/InstructionSet.md. The COMPARISONS are the only entries with no `d`, because they branch
       instead of producing a value - their operands 0 and 1 are both sources and the label is operand 2.
       Stating the destination rather than inferring it from an absent `0` keeps the failure direction
       safe: forget the marker on a new operator and it reads as "not foldable", never as "straight line". */
    INLINE_IMM_OK = {};
    map(INLINE_IMM_OK,
        '=',    'd1',  ':=',     'd1',                     /* MOVi   int(d) #int          */
        '=abs', 'd1',  '=floor', 'd1', '=itof', 'd1', '=ftoi', 'd1',  /* ABSi/FLOf/ITOF/FTOI */
        '+',   'd12',  '-',     'd12', '*',    'd12', '/',    'd12', '%', 'd12',
        '|',   'd12',  '&',     'd12', '^',    'd12',
        '<<',  'd12',  '>>',    'd12', '>>>',  'd12',
        '<',    '01',  '<=',     '01', '>',     '01', '>=',   '01', '==', '01', '!=', '01',
    ...
    (function () {                                            /* the INLINE_PURE seeding promised above */
        for (var k in INLINE_IMM_OK) {
            if (Object.prototype.hasOwnProperty.call(INLINE_IMM_OK, k)) INLINE_PURE[k] = 1;
        }
    })();

    /* How many immediate sources an operator CALCULATES into position 0, or undefined if it calculates
       nothing - which is exactly the operators with no `d`. One derivation serves both the constant fold
       and the straight-line test, so the two can never disagree about what "produces a value" means. */
    inlineFoldWidth = function (op) {
        var pos = INLINE_IMM_OK[op];
        return (pos === undefined || pos.charAt(0) !== 'd' ? undefined : pos.length - 1);
    };

    /* May this operand carry an immediate in EVERY place the body uses it? One bad use - a SWCH value,
       an array base, a POKE address - spoils it, and materialising it there while substituting elsewhere
       would need the MOV anyway. For a folded TEMPORARY, a destination position 0 is the write being
       replaced, so it is not a use; the `d` marker says so per operator rather than per call site. */
    function takesImmediate(body, name, isTemp) {
        for (var i = 0; i < body.length; ++i) {
            if (body[i].operator === ';') continue;                /* comment prose, not operands */
            var okPos = INLINE_IMM_OK[body[i].operator];
            for (var k = (isTemp && ('' + okPos).charAt(0) === 'd' ? 1 : 0); k < 3; ++k) {
    ...
    rejectInlineAddress = function (rec, operand) {
        if (operand == null) return;
        var name = '' + operand;
        if (name.charAt(0) !== '&') return;
        var fn = symbols.functions[name = name.substr(1)];
        if (fn !== undefined && fn.inline !== undefined) {
            /* Deliberately checked HERE and not where the operand is minted: `lookup` also runs for a
               CALL, which expands rather than taking an address, so rejecting there rejects `f(1)` too.
               By the time a record reaches output every call has expanded, so a surviving `&name` IS an
               address-take. The position rides the record (as `labelSource`/`gotoOffset` do), because
               passing `undefined` rendered this with no code, line or caret - which is what Impala 2's
               harness refuses. */
            var at = inlineAddrAt[name] || {};
            fail('Cannot take the address of the inline function ' + name,
                    at.s, at.o, 'E435',
                    'an inline function has no out-of-line copy; drop `inline` to use it as a value');
        }
    };

    /* Can this ONE record write a location it does not NAME? That is the whole soundness question, both
       for a body (can it disturb a caller's local?) and for the marshalling scan below (can it disturb a
       read we are about to defer past it?). One predicate, so the two cannot drift apart. */
    recordIsOpaque = function (m) {
        var op = m.operator;
        if (op === ';' || op.substr(0, 3) === '<> ') return false;  /* prose, or a fold writing only `<X>` */
        if (op === '[]=' || op === '*=') {                          /* POKE: nameable only as `&global` */
            return (('' + m.operands[0]).charAt(0) !== '&');
        }
        return (op === '()' || op === 'copy'                        /* a call, or a computed-dest COPY */
                || !INLINE_PURE[op]);                      /* unrecognised -> assume the worst */
    };

    bodyIsOpaque = function (body) {
        for (var i = 0; i < body.length; ++i) if (recordIsOpaque(body[i])) return true;
        return false;
    };

    /* Snapshot a body before flushMetaCode() clears it, operand-for-operand. The `;` source comments are
       KEPT, so an expansion can re-emit them. (There is no RETU to drop: it is emitted after this runs.) */
    captureInlineBody = function (name, sourceCode, sourceOffset) {
        var entry = symbols.functions[name];
        ...
        for (var i = 0; i < metacode.length; ++i) {
            ...
            body.push({ operator: m.operator, type: m.type, inlineFold: m.inlineFold,
                    operands: [ m.operands[0], m.operands[1], m.operands[2] ] });
            for (var mo = 0; mo < 3; ++mo) {        /* the callee numbers its temporaries from 0; `%<X>`
                                                       parses as NaN and so drops out of the compare */
                var mv = m.operands[mo];
        ...
        for (var p = 0; p < sig.params.length; ++p) {
            ...
            litOK.push(takesImmediate(body, params[p], false));
        }
        /* Constant folding inside the body (see expandInline) propagates a value from one instruction
           to the next, which is only sound along a straight line: a label is a join point and a branch
           can skip the write. Rather than model that, only fold bodies that are PURE ARITHMETIC - every
           instruction a move or a calculation. That is exactly the shape worth folding anyway. */
    ...
    /* Replay a captured body at a call site. The arguments are ALREADY marshalled into the call
       window, so materialising is the default and substitution is a deletion: if the instruction that
       filled a window slot is a plain move of a literal (or of a local, when the body is transparent),
       drop it and use its source directly. Rules 2 and 4 of the spec then need no code at all - a
       computed argument and a global argument both just keep their window slot. */
    expandInline = function (info, base, retSlots, argCount, mark) {
        var subst = {}, i, j, k;
        for (k = 0; k < argCount; ++k) {
            var slot = '%' + (base + retSlots + k);
            var src = slot;
            for (j = metacode.length - 1; j >= mark; --j) {   /* only THIS call's own marshalling */
                var m = metacode[j];
                if (m.operator === undefined) continue;
                if (m.operands[0] !== slot) {
                    /* Deferring the read past this record means the body sees whatever it did. A CALL
                       writes its own window (naming the base at operand 1, so the test above misses it),
                       a POKE writes through a pointer, and a nested inline expansion emits either inline
                       with no CALL record at all. All of them are exactly `recordIsOpaque`. */
                    if (recordIsOpaque(m)) break;
                    continue;
                }
            ...
            subst[info.params[k]] = src;
        }
        if (info.ret !== undefined) subst[info.ret] = '%' + base;   /* the window's output slot */
        var runs = [];                              /* every run borrowed below, released in reverse at the end */
        function borrowRun(n) {                     /* an array local and the callee's own temporaries both
                                                       need their slots ADJACENT */
            var b = (n > 1 ? borrowForCall()
                           : parseInt(borrow('%').substr(1), 10));
            for (var q = 1; q < n; ++q) claimSlot(b + q);   /* slot 0 came with the borrow */
            runs.push([ b, n ]);
            return b;
        }
        ...
        /*
            The callee's declared locals become REAL frame locals of the caller, bracketed by
            SCOP / ENDS (GAZL 2). The assembler then owns their placement, exactly as it does for
            an ordinary local - which is the whole point: it resolves `*.z.Struct` before assigning
            offsets, so a host-owned size lands correctly, whereas a compile-time slot count could
            only ever be Impala's guess at it.

            Sibling expansions overlay, so the frame cost is the LARGEST expansion in a function,
            not the sum. That is sound because an expansion's locals are dead once its body ends:
            the result leaves through the call window, never through a local.

            The transient block below is a different matter and stays transient - it carries the
            body's own `%k` slots, including CALL windows, whose base must be a transient.
        */
        if (info.locals.length > 0) {
            var _tab = (typeof TAB !== 'undefined' ? TAB : '\t');    /* same shape declare() renders */
            emitLine(_tab + '\tSCOP');
            for (k = 0; k < info.locals.length; ++k) {
                var lv = info.locals[k];
                var lname = lv.name + tag;                      /* `$tmp` -> `$tmp_i3`, unique per expansion */
                /* `lv.size` is repeated verbatim, however the extent was arrived at: an array's is a
                   `.x.` constant the callee's declaration named once, a struct's is `*.z.Struct`. Both
                   are SYMBOLS the assembler resolves, so nothing has to be re-derived here - which is
                   the point, since a fold lives in a scratch this head has no claim on. */
                emitLine(_tab + lname + ':\t' + declRow(lv.kind, lv.type, lv.size));
                subst[lv.name] = lname;
            }
            emitLine(_tab + '\tENDS');
        }

        var scratchMap = {};                    /* `<X>` folds carried in from an inner expansion */
        function mapScratch(sc) {
            if (!Object.prototype.hasOwnProperty.call(scratchMap, sc)) scratchMap[sc] = borrow('<');
            return scratchMap[sc];
        }
        var tCount = info.maxTransient + 1;      /* one run keeps every window inside the body intact */
        var tBase = (tCount > 0 ? borrowRun(tCount) : -1);
        var symOffsets = {};

        var constMap = {}, foldScratch = [];     /* callee values that turned out to be compile-time */
        if (info.immOK === undefined) info.immOK = {};   /* takesImmediate scans the whole body, and the body
                                                            never changes after capture - so the per-name answer
                                                            is durable across every expansion of this callee */
        function tempImmOK(name) {
            var s = '' + name;
            if (!Object.prototype.hasOwnProperty.call(info.immOK, s)) {
                info.immOK[s] = takesImmediate(info.body, name, true);
            }
            return info.immOK[s];
        }
        function constFold(op) {                 /* read a value the fold above already resolved */
            return (op !== undefined && Object.prototype.hasOwnProperty.call(constMap, '' + op)
                    ? constMap['' + op] : op);
        }
        function isConstOperand(op) {            /* `#42` immediate, or `<A>` compile-time scratch */
        ...
        function mapOperand(op) {
            ...
            if (s.charAt(0) === '%') {
                if (s.charAt(1) === '<') return '%' + mapScratch(s.substr(1));   /* %<X>: rename only */
                return '%' + (tBase + parseInt(s.substr(1), 10));   /* a slot number, shifted as a block */
            }
            if (s.length === 3 && s.charAt(0) === '<') return mapScratch(s);   /* a bare `<X>` */
            ...
            /* A frame place must be resolved BEFORE the compound-scratch rule below: a nested struct
               field arrives as `$v:<A>` (base plus a folded offset scratch), and renaming the scratch
               without mapping the base would leave a local the caller never declared. */
            ...
            if (colon > 0 && Object.prototype.hasOwnProperty.call(subst, s.substr(0, colon))) {
                ...
                var off = s.substr(colon + 1);
                if (off.length === 3 && off.charAt(0) === '<') off = mapScratch(off);
                if (mapped.charAt(0) !== '%') return mapped + ':' + off;
                /* GAZL has no `%N:offset`. A numeric element offset folds straight into the slot
                   number; a struct field's offset is a SYMBOL (`.o.S.f`) that only the assembler can
                   resolve, so fold it there instead and index the transient symbolically - `%<X>` is
                   legal, and the whole fold is assemble-time, costing nothing at run time. */
                var n = parseInt(off, 10);
                if (!isNaN(n) && ('' + n) === off) return '%' + (parseInt(mapped.substr(1), 10) + n);
                if (!Object.prototype.hasOwnProperty.call(symOffsets, s)) {
                    var sc = borrow('<');   /* minted before the record that reads it, so push it now */
                    metacode.push({ operator: '<> +', type: 'i', settled: true, inlineFold: true,
                            operands: [ sc, '#' + mapped.substr(1), '#' + off ] });
                    symOffsets[s] = '%' + sc;
                }
                return symOffsets[s];
            }
            var ts = trailingScratch(s);               /* `*<A>`, `#<D>`, `&g:<D>` */
            if (ts !== null) {
                return s.substr(0, s.length - 3) + mapScratch(ts);
            }
            return s;
        }
        for (i = 0; i < info.body.length; ++i) {
            ...
            var d0 = mapOperand(b.operands[0]), fw = inlineFoldWidth(b.operator);
            var s1 = (b.inlineFold ? '#' + (tBase + parseInt(('' + b.operands[1]).substr(1), 10))
                                   : constFold(mapOperand(b.operands[1])));
            var s2 = constFold(mapOperand(b.operands[2]));

            /* CONSTANT FOLDING. Sources that are all compile-time constants make the RESULT one too, so
               keep it in a `<X>` scratch instead of a transient. GAZL would otherwise fold the maths at
            ...
            if (info.straightLine && !b.inlineFold && isConstOperand(s1) && fw !== undefined
                    && (fw === 1 ? s2 === undefined : isConstOperand(s2))
                    && ('' + d0).charAt(0) === '%' && ('' + d0) !== '%' + base
                    && tempImmOK(b.operands[0])) {
                if (b.operator === '=' || b.operator === ':=') {
                    constMap['' + d0] = s1;            /* a move of a constant is pure propagation */
                } else {
                    var fsc = borrow('<');
                    foldScratch.push(fsc);
            ...
            delete constMap['' + d0];                  /* written with a run-time value again */
            metacode.push({ operator: b.operator, type: b.type, settled: true,
                    inlineFold: b.inlineFold, operands: [ d0, s1, s2 ] });
        }
        for (k = 0; k < foldScratch.length; ++k) returnBack(foldScratch[k]);
        for (var sm in scratchMap) {
            if (Object.prototype.hasOwnProperty.call(scratchMap, sm)) returnBack(scratchMap[sm]);
        }
        for (var so in symOffsets) {
            if (Object.prototype.hasOwnProperty.call(symOffsets, so)) returnBack(symOffsets[so]);
        }
        for (k = runs.length - 1; k >= 0; --k) {
            for (var rs = runs[k][1] - 1; rs >= 0; --rs) returnBack('%' + (runs[k][0] + rs));
        }
    };
    ...
    emitStructLayout = function (name) {
        ...
        emitLine(T + '! MOVi <a> #0' + T + '; layout of struct ' + name);
        for (var i = 0; i < s.fields.length; ++i) {
            var f = s.fields[i];
            emitLine(fieldSymbol(name, f.name) + ':' + T + '! DEFi #<a>');
            if (f.type === 'S') {                                 /* nested by-value struct */
                emitLine(T + '! ADDi <a> #<a> #' + extentSymbol(f.struct));
                ...
                if (isStructAtom(f.elem)) {              /* count * element size, folded now - except at
                    ...
                    if (constInt('#' + f.size) === 1) {
                        ...
                        emitLine(T + '! MULi <t> #' + f.size + ' #' + esym);
                        words = '<t>';
                    }
                }
                emitLine(x + ':' + T + '! DEFi #' + words);
                ...
                for (var d = 0; f.dims !== undefined && d < f.dims.length; ++d) {
                    emitLine(axisSymbol(f.name, name, d) + ':' + T + '! DEFi #' + f.dims[d]);
                }
                if (constInt('#' + f.size) === undefined) {
                    ...
                    emitLine(T + '! GEQi #' + x + ' #0 @' + neg);
                    emitLine(T + '! FAIL extent of ' + name + '.' + f.name
                            + ' is negative, which would run the struct layout backwards');
                    emitLine(neg + ':' + T + '! ADDi <a> #<a> #' + x);
                    continue;
                }
                emitLine(T + '! ADDi <a> #<a> #' + x);
            } else {                                              /* scalar (int/float/ptr/funcptr) */
                emitLine(T + '! ADDi <a> #<a> #1');
            }
        }
        emitLine(extentSymbol(name) + ':' + T + '! DEFi #<a>');
    };
    ...
    /* The symbol naming an array's extent in WORDS - the `*size` operand, not the element count (a
       ...
       See design/gazl/SymbolNamespace.md. (This branch carried `.x.` from the fork point; Impala 2 renamed it
       to `.z.` on the reasoning above, and that rename arrives with this merge.) */
    extentSymbol = function (name, owner) {
        return '.z.' + (owner !== undefined ? owner + '.' : '') + name;
    };

    /* The mnemonic-plus-value tail of a declaration row (`DATi #60`, `LOCA *.z.V`) - declare() renders
       it for every ordinary declaration and expandInline for the SCOP-scoped locals it pastes, so the
       one format lives here and cannot diverge between them. */
    declRow = function (kind, type, value) {
        return replace(kind, '?', TYPE_SUFFIXES[type] || '')
    ...
    buildStructInit = function (structName, items, out, sourceCode, sourceOffset) {
        ...
        /* The HOST owns an extern struct's field offsets and its size, so a positional DATA row is a
           ...
           Under `--gazl2` (`out.seeks` set) the guess is gone: every word is PLACED at its `.o.` symbol
           with a SEEK region, so a host-owned layout is as initializable as a compiler-owned one. */
        if (isExternStruct(structName) && out.seeks === undefined) {
            blockInitFrom(out,
                    ...
                    'a host-owned struct is initialized by the host - leave it zero-filled here; '
                            + 'or compile with --gazl2, which places every word at its host-defined '
                            + '.o. offset with a SEEK region (needs a GAZL 2 engine)');
        }
        ...
        for (var fi = 0; fi < fields.length; ++fi) {
            ...
            if (f.type === 'S') {
                if (out.seeks !== undefined) { out.path.push(fieldSymbol(structName, f.name)); }
                buildStructInit(f.struct, (item && item.braced) || [], out, sourceCode, sourceOffset);
                if (out.seeks !== undefined) { out.path.pop(); }
            } else if (f.type === 'A') {
                ...
                if (out.seeks !== undefined) {
                    out.seeks.push({ at: out.length,
                            parts: out.path.concat(fieldSymbol(structName, f.name)),
                            /* an extern struct mints no `.z.E.field`, so its region is fenced by the
                               section instead of an extent the host never published */
                            extent: (isExternStruct(structName) ? undefined
                                    : extentSymbol(f.name, structName)),
                            type: coarseType(f.elem) });
                }
                if (fillArray(f, structName, (item && item.braced) || [], out,
                        sourceCode, sourceOffset)
                        && out.blocked === undefined && out.seeks === undefined) {
                    blockInitFrom(out,
                            ...
                            'leave every field after ' + f.name + ' zero here - ' + f.name + ' itself may '
                                    ...
                                    + 'or compile with --gazl2, which places each field with a SEEK '
                                    + 'region (needs a GAZL 2 engine)');
                }
            } else {
                if (out.seeks !== undefined) {
                    out.seeks.push({ at: out.length,
    ...
    fillEntry = function (out, ev, structEl, elemDesc, name, sourceCode, sourceOffset) {
        if (structEl) {
            if (out.seeks !== undefined) {
                /* element k of a struct array sits k strides past the array's anchor; the term stays
                   SYMBOLIC ({mul, sym}) here and is folded only when its region is rendered, so the
                   scratch pool is never held across the walk (emitInitData owns the folds) */
                out.path.push({ mul: out.elemIndex++, sym: extentSymbol(descName(elemDesc)) });
            }
    ...
    fillArray = function (slot, owner, tree, out, sourceCode, sourceOffset) {
        ...
        var savedElem;
        if (out.seeks !== undefined) {                        /* linear element index for struct elements' stride
                                                                 terms; saved so a nested struct array restores
                                                                 its outer array's count */
            savedElem = out.elemIndex;
            out.elemIndex = 0;
        }
        fillAxis(slot, axes, 0, expected, tree, out, sourceCode, sourceOffset);
        if (out.seeks !== undefined) { out.elemIndex = savedElem; }
        if (expected !== undefined && out.seeks === undefined) {   /* the one bound a symbolic extent still needs;
                                                                      a SEEK region states it as `*extent` and the
                                                                      assembler checks the fill against it directly */
    ...
    /* The coarse type a TYPED data row may state, from a bare type code: i/f/p, plus t under --gazl2
       (on GAZL 1 funcptr rows stay the untyped DATA - byte-identical - because DATt does not exist
       there). The ONE spelling of "which types get typed rows", shared by every marker and row site. */
    typedRowType = function (t) {
        return (t === 'i' || t === 'f' || t === 'p' || (t === 't' && gazl2) ? t : undefined);
    };
    ...
    coarseType = function (desc) {
        ...
        return typedRowType(descTypeElem(desc).type);
    };
    ...
    emitInitData = function (ops, sourceCode, sourceOffset) {
        ...
        if (ops.seeks !== undefined) {
            /* --gazl2: PLACED data. One SEEK region per leaf field, offset composed from the marker's
               symbolic parts - `.o.` symbols plus {mul, sym} element-stride terms - folded HERE through
               the same pool-managed scaleByStride/foldOffset every access site uses, so the walk itself
               never held a scratch. Rows come out typed per field (`DATi`/`DATf`/`DATp`): a region spans
               one leaf, so the mixed-type row `DATA` existed for cannot arise inside one. */
            for (var s = 0; s < ops.seeks.length; ++s) {
                var m = ops.seeks[s];
                var end = (s + 1 < ops.seeks.length ? ops.seeks[s + 1].at : ops.length);
                if (end > ops.length) { end = ops.length; }
                var words = ops.slice(m.at, end);
                while (words.length > 0 && isZeroWord(words[words.length - 1])) {
                    words.pop();                              /* a region zero-fills its own tail */
                }
                if (words.length === 0) {
                    continue;                                 /* all-zero region: it IS what SEEK would claim */
                }
                var atoms = [];
                for (var k = 0; k < m.parts.length; ++k) {
                    var p = m.parts[k];
                    if (typeof p === 'object') {
                        if (p.mul !== 0) {                    /* element 0 sits at the anchor itself */
                            atoms.push(scaleByStride(p.mul, p.sym));
                        }
                    } else {
                        atoms.push(p);
                    }
                }
                var off = foldOffset(atoms);
                declare('SEEK', 'globals', undefined, 'i', true,
                        ':' + off + (m.extent !== undefined ? ' *' + m.extent : ''),
                        sourceCode, sourceOffset);
                returnBack(off);                     /* buried mid-operand when an extent follows, so
                                                                 declare's own trailing-scratch return misses it;
                                                                 returnBack de-dupes, so the no-extent case is safe */
                emitDataRows(words, (m.type !== undefined ? 'DAT?' : 'DATA'), (m.type || 'i'),
                        sourceCode, sourceOffset);
            }
            return;
        }
        var coarse = coarseType(ops.target);
        emitDataRows(ops, (coarse !== undefined ? 'DAT?' : 'DATA'), (coarse || 'i'),
                sourceCode, sourceOffset);
    };
    ...
    emitDataRows = function (words, row, type, sourceCode, sourceOffset) {
        var line = '';
        for (var i = 0; i < words.length; ++i) {
            if (line !== '' && (words[i].charAt(0) === '<' || line.charAt(0) === '<'
                    || (line + ' ' + words[i]).length >= 55)) {
...
var _sn = strideStruct(field.elem);
    ...
    binaryOp = function (operator, leftx, rightx,
        ...
        if (operator === '=[]') {
            ...
            if (diff) {
                ...
                operator = (funcDiff && gazl2 ? 'dt' : 'd');   /* a t local is no DIFp operand - the one DIF
                                                                           site TYPE_SUFFIXES cannot reach, DIF having
                                                                           no `?` (int minus is SUBi) */
    ...
    function declare(kind, scope, name, type, readonly, value, sourceCode, sourceOffset, comment, elem) {
        ...
        /* An `inline` function has no out-of-line copy, so its FRAME declarations belong to nobody: each
           expansion re-declares them in its own SCOP. Emitting them here put bare `$r: OUTi` rows at file
           scope, ahead of the first FUNC, which the assembler rejects outright. Only `locals` is skipped -
           a `defines` row like `.z.f.buf: ! DEFi #3` is a compile-time constant the expansions REFER to,
           so it must still be emitted, exactly once, out here. */
        if (kind !== undefined && !(inlineMode && scope === 'locals')) {
            flushMetaCode('');
    ...
    flushMetaCode = function (prefix) {
        ...
        for (var i = 0; i < metacode.length; ++i) {
            ...
            for (var ia = 0; ia < 3; ++ia) rejectInlineAddress(rec, rec.operands[ia]);
            ...
            if (op === '<--') {
                ...
                if (nextLabel !== TABstr) {
                    emitLine(prefix + nextLabel + 'NOOP');
                }
            ...
            emitLine(prefix + nextLabel + gInstr + ' ' +
                   formatOperand(rec.operands[0]) + ' ' +
                   formatOperand(rec.operands[1]) + ' ' +
                   formatOperand(rec.operands[2]) + nextComment);
        ...
        if ((p = asFn)) {
            if (p.type === 'N') {
                ...
                makeMeta(x, ':=', 't', undefined,
                                  '&' + name, undefined);
                /* Remember WHERE, keyed by name: `assign` rebuilds the record before it reaches output,
                   so stamping the slot does not survive, and a function name is unique anyway. */
                if (p.inline !== undefined) {
                    inlineAddrAt[name] = { s: sourceCode, o: sourceOffset };
                }
            }
            setElem(x, undefined);
            return;
    ...
    start = function () {
        ...
        if (strings.a.length > 0) {
            ...
            emitLine('\t! EQUi #DEBUG #0 @.noAssertStrings');
...
};function root(){return (function(){var _b=_i;return _()&&(function(){ start(); ; return true})()&&((function(){while((function(){var _b=_i;return (function(){ declOffset = _i; declSource = _s; ; return true})()&&(function(){var _b=_i;return ImportDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||ExportDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||FuncDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||StructDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||FuncTypeDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||ExternDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||ConstDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||GlobalDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]===";")&&(++_i,true)&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(function(){var _l=_i,_lv=_val,_x=(!!_s[_i])&&(++_i,true);_i=_l;_val=_lv;return !_x})()&&(function(){ end(); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function ImportDecl(){var $path=newMetaSlot(),_sv$path;return (function(){var _b=_i;return IMPORT()&&_()&&((_sv$path=_val,_val=$path,(StringLiteral()))&&($path=_val,_val=_sv$path,true)||(_val=_sv$path,false))&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
...
function StructDecl(){var $nmAt,$id=newMetaSlot(),_sv$id,$sname,$f=newMetaSlot(),_sv$f;return (function(){var _b=_i;return STRUCT()&&_()&&(function(){ $nmAt = _i; ; return true})()&&((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(function(){ $sname = $id; beginStruct($id, _s, $nmAt); ; return true})()&&(_s[_i]==="{")&&(++_i,true)&&_()&&((function(){while((function(){var _b=_i;return ((_sv$f=_val,_val=$f,((function(){var _b=_i;return ArrayDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||VarDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)})()))&&($f=_val,_val=_sv$f,true)||(_val=_sv$f,false))&&(function(){ addStructField($sname, { name: $f.name, type: $f.type, elem: $f.elem, struct: $f.struct, size: $f.size, dims: $f.dims }, _s, _i); ; return true})()&&((function(){var _b=_i;return (_s[_i]===";")&&(++_i,true)&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(_s[_i]==="}")&&(++_i,true)&&_()&&(function(){   endStruct($sname); /* Publish the real layout with TYPES so a signature-row consumer can check an `extern struct` declaration of the same name against it - the .o./.z. constants alone carry no types. */ emitStandaloneSignatureComment( structSignatureRow($sname, false, sourceName, _s, declOffset)); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function FuncTypeDecl(){var $nmAt,$id=newMetaSlot(),_sv$id,$ftname,$p=newMetaSlot(),_sv$p,$r=newMetaSlot(),_sv$r;return (function(){var _b=_i;return FUNCTYPE()&&_()&&(function(){ $nmAt = _i; ; return true})()&&((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(function(){ $ftname = $id; beginFuncType($id, _s, $nmAt); ; return true})()&&(_s[_i]==="(")&&(++_i,true)&&_()&&((function(){var _b=_i;return ((_sv$p=_val,_val=$p,(TypeDeclr()))&&($p=_val,_val=_sv$p,true)||(_val=_sv$p,false))&&(function(){ addFuncTypeParam($ftname, $p.type, $p.elem, $p.struct, $p.words, $p.name, _s, _i); ; return true})()&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_()&&((_sv$p=_val,_val=$p,(TypeDeclr()))&&($p=_val,_val=_sv$p,true)||(_val=_sv$p,false))&&(function(){ addFuncTypeParam($ftname, $p.type, $p.elem, $p.struct, $p.words, $p.name, _s, _i); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&(_s[_i]===")")&&(++_i,true)&&_()&&((function(){var _b=_i;return RETURNS()&&_()&&((_sv$r=_val,_val=$r,(TypeDeclr()))&&($r=_val,_val=_sv$r,true)||(_val=_sv$r,false))&&(function(){ addFuncTypeReturn($ftname, $r.type, $r.elem, $r.struct, $r.words, _s, _i); ; return true})()&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_()&&((_sv$r=_val,_val=$r,(TypeDeclr()))&&($r=_val,_val=_sv$r,true)||(_val=_sv$r,false))&&(function(){   /* PARKED for Impala 3.0 - see design/ParkedFeatures.md */ fail('Multiple return values are not supported in Impala 2.0', _s, _i, 'E428', 'declare a single return type for this funcptr type'); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&(function(){ endFuncType($ftname); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function TypeDeclr(){var $base=newMetaSlot(),_sv$base,$desc,$id=newMetaSlot(),_sv$id;return (function(){var _b=_i;return ((_sv$base=_val,_val=$base,(TypeBase()))&&($base=_val,_val=_sv$base,true)||(_val=_sv$base,false))&&(function(){ _val = {}; $desc = $base; ; return true})()&&((function(){while((function(){var _b=_i;return POINTER()&&_()&&(function(){ $desc = pointerDesc($desc); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&((function(){var _b=_i;return ((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(function(){ _val.name = $id; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&(function(){ var td = descTypeElem($desc); if (td.type === 'S') { _val.type = 'S'; _val.struct = td.elem; _val.elem = undefined; _val.words = structWords(td.elem); } else { _val.type = td.type; _val.elem = td.elem; _val.struct = undefined; _val.words = (td.type === 't' ? 1 : undefined);   /* scalar funcptr: one word */ } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function FuncDecl(){var $inl,$inlAt,$fnAt,$id=newMetaSlot(),_sv$id,$inp=newMetaSlot(),_sv$inp,$out=newMetaSlot(),_sv$out,$v=newMetaSlot(),_sv$v,$,$loc=newMetaSlot(),_sv$loc;return (function(){var _b=_i;return (function(){ $inl = false; ; return true})()&&((function(){var _b=_i;return INLINE()&&(function(){ $inl = true; $inlAt = _i; ; return true})()&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&FUNCTION()&&_()&&(function(){ $fnAt = _i; ; return true})()&&((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(_s[_i]==="(")&&(++_i,true)&&_()&&(function(){ /* E439 (`inline` is parked) belongs to the Impala 2 line, which must run on GAZL 1.0 engines. This branch HAS the `SCOP`/`ENDS` an expansion needs, so the feature is live here and the rejection does not come across. */ inlineMode = $inl; inlineLocals = []; if ($inl && symbols.functions[$id] !== undefined) { fail('The inline function ' + $id + ' was already declared', _s, $fnAt, 'E436', 'an inline function emits no symbol, so it cannot be forward declared or redeclared'); } if ($inl && exportNext) { fail('An inline function cannot be exported', _s, $fnAt, 'E434', 'there is no out-of-line copy to link against; a caller needs the source, via `import`'); } assert(validateStock('%')); assert(validateStock('<')); /* every compile-time scratch borrowed by the previous function/globals must be back in the pool at this clean boundary - catches offset-scratch leaks at the source */ assert(stock['<'].length === counters['<'], 'compile-time scratch leak before ' + $id + ': ' + (counters['<'] - stock['<'].length) + ' unreturned'); if (!inlineMode) { emitLine(''); emitLine(';-----------------------------------------------------------------------------'); } /* Buffer this function's head until the body is parsed - see emitLine. Nothing is added to it yet, so the replay below is byte-identical to emitting directly. */ headSink = []; /* declare the function symbol */ declare( undefined, 'functions', $id, 'U', true, undefined, _s, $fnAt   /* the NAME, not the `(` this action already ate */ ); var entry = symbols.functions[$id]; if (entry) { if (!entry.signature) { entry.signature = {}; } entry.signature.params = []; entry.signature.returns = '?'; entry.signature.returnElem = undefined; entry.signature.returnName = undefined; entry.signature.sourceCode = _s; entry.signature.sourceOffset = declOffset; entry.signature.sourceName = sourceName; entry.signature.returnResolved = false; entry.pendingReturnPlaceholder = undefined; entry.pendingReturnDeclaration = undefined; } ; return true})()&&((_sv$inp=_val,_val=$inp,(ArgsDecl()))&&($inp=_val,_val=_sv$inp,true)||(_val=_sv$inp,false))&&(_s[_i]===")")&&(++_i,true)&&_()&&(function(){var _b=_i;return RETURNS()&&_()&&((_sv$out=_val,_val=$out,(VarDecl()))&&($out=_val,_val=_sv$out,true)||(_val=_sv$out,false))&&(function(){ var entry = symbols.functions[$id]; if (entry) { entry.pendingReturns = undefined; /* `$out.at` is the NAME; `_i` here has already skipped past the whole declarator onto the next clause, which put E463's caret on `locals`. */ addReturn(entry, $out.name, $out.type, $out.elem, $out.size, _s, $out.at, $out.struct, $out.words); } ; return true})()&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_()&&((_sv$v=_val,_val=$v,(VarDecl()))&&($v=_val,_val=_sv$v,true)||(_val=_sv$v,false))&&(function(){   /* PARKED for Impala 3.0 - see design/ParkedFeatures.md */ fail('Multiple return values are not supported in Impala 2.0', _s, _i, 'E428', 'return one value, or pass extra results back through pointer out-parameters'); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(function(){ var entry = symbols.functions[$id]; if (entry && entry.signature) { var rl = entry.pendingReturns; entry.signature.returnList = rl; entry.signature.returnCount = rl.length; entry.signature.returns = rl[0].type; entry.signature.returnElem = rl[0].elem; entry.signature.returnName = rl[0].rawName; entry.signature.returnStruct = rl[0].struct; var _rw = 0;                    /* total output-window words (struct returns span >1) */ for (var _wi = 0; _wi < rl.length; ++_wi) _rw += (rl[_wi].type === 'S' ? rl[_wi].words : 1); entry.signature.returnWords = _rw; resolveFunctionReturnType($id, rl[0].type, _s, _i); } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||(function(){ /* implicit 1-word return: even void functions expose a single-word PARA so legacy call sites keep a deterministic return slot and the JSPEG output matches the historical PPEG layout. */ var entry = symbols.functions[$id]; if (entry) { entry.pendingReturns = undefined; entry.pendingReturnPlaceholder = { sourceCode: _s, sourceOffset: _i }; } if (entry && entry.signature) { entry.signature.returns = 'V'; entry.signature.returnElem = undefined; entry.signature.returnName = undefined; entry.signature.returnCount = 0; entry.signature.returnList = []; resolveFunctionReturnType($id, 'V', _s, _i); } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(function(){ /* declare input parameters */ var entry = symbols.functions[$id]; if (entry && entry.signature) { entry.signature.params = copyParams($inp, $inp.n); } emitFunctionSignature($id, _s, _i); if (entry) { if (entry.pendingReturns && entry.pendingReturns.length > 0) { for (var _ri = 0; _ri < entry.pendingReturns.length; ++_ri) { var ret = entry.pendingReturns[_ri]; rejectByValueStruct(ret.type, ret.struct, ret.rawName, true, ret.sourceCode, ret.sourceOffset); { declare( 'OUT?', 'locals', ret.name, ret.type, false, ret.size, ret.sourceCode, ret.sourceOffset, undefined, ret.elem ); } } entry.pendingReturns = undefined; } else if (entry.pendingReturnPlaceholder) { var placeholder = entry.pendingReturnPlaceholder; declare( 'PARA', 'locals', undefined, '?', false, '*1', placeholder.sourceCode, placeholder.sourceOffset ); entry.pendingReturnPlaceholder = undefined; } } iterate($inp, function (p) { rejectByValueStruct(p.type, p.struct, p.name, false, _s, p.at); { declare( 'INP?', 'locals', '$' + p.name, p.type, true, (p.size !== undefined ? '*' + p.size : undefined), _s, p.at, undefined, p.elem ); } }); ; return true})()&&((function(){var _b=_i;return LOCALS()&&_()&&((_sv$loc=_val,_val=$loc,(LocalsDecl()))&&($loc=_val,_val=_sv$loc,true)||(_val=_sv$loc,false))&&(function(){ iterate($loc, function (v) { /* Compute the allocation size ONCE: the declaration below needs it, and for an `inline` function so does every expansion site, which re-declares the local inside its own SCOP. Calling arrayAllocSize twice would borrow two scratches - and now that it also names each axis, redeclare every `.d.` symbol too. An array's size is a NAMED `.z.` extent, so it is a symbol every expansion can repeat verbatim - no re-rendering the pieces it was folded from. */ var isStruct = (v.type === 'S'); var kind = (isStruct ? 'LOCA' : 'LOC?'); var type = (isStruct ? 'S' : v.type); var elem = (isStruct ? structDesc(v.struct) : v.elem); var size = (isStruct ? structAllocSize(v.struct, _s, v.at) : (v.type === 'A' ? arrayAllocSize(v.elem, v.size, v.name, $id, v.dims) : (v.words !== undefined ? '*' + v.words : undefined))); if (inlineMode) { inlineLocals.push({ name: '$' + v.name, kind: kind, type: type, size: size, elem: elem }); } declare(kind, 'locals', '$' + v.name, type, false, size, _s, v.at, undefined, elem); if (v.type === 'A') {   /* the ONLY place the owning function's name is in scope, so what a later subscript needs to bounds-check this array is recorded here - see arrayExtent's shape */ symbols.locals['$' + v.name].extent = arrayExtent(v.name, $id, v.name, v.size, v.dims); } }); returnExtent('locals of ' + $id, $loc); /* the count and axis scratches ArrayDecl held for this clause (see there) */ ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&(function(){ /* The head/body separator is emitted at the REPLAY below, not here: an inline expansion appends its SCOP block to the head while the body is being parsed, so anything written here would end up above those declarations. */ tailFn = $id;   /* in an inline body E468 throws before this is read */ ; return true})()&&Block()&&(function(){ /* wrap-up body */ /* The closing RETU goes in BEFORE the pass, so the pass can see it: `goto out;` where `out:` is the end-of-body label is Impala's only early-exit idiom, and it lands exactly here. NOT for an `inline` body - that is captured for substitution, and an expansion pasted mid-caller must not return. */ if (!inlineMode) { emit('--^', undefined, undefined, undefined, undefined); } checkReturnAssigned(symbols.functions[$id]); processBranches(); /* The body (and every inline expansion in it) is now known, so the head is final: replay it, then let flushMetaCode write the body straight out. */ var _head = headSink;   /* always an array here: FuncDecl opened the sink unconditionally */ headSink = null; for (var _h = 0; _h < _head.length; ++_h) emitLine(_head[_h]); if (inlineMode) {                       /* no out-of-line copy: keep the body, emit nothing */ captureInlineBody($id, _s, _i); metacode.length = 0; } else { emitLine(';-----------------------------------------------------------------------------'); flushMetaCode('\t');   /* the RETU went in above the pass */ } emittedGuards = {};   /* per function: labels are too */ prune(symbols.locals); labelCounter = 0; if (!inlineMode) emitLine(''); inlineMode = false; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function ExternDecl(){var $at,$id=newMetaSlot(),_sv$id,$sname,$f=newMetaSlot(),_sv$f,$nmAt,$inp=newMetaSlot(),_sv$inp,$out=newMetaSlot(),_sv$out,$a=newMetaSlot(),_sv$a,$d=newMetaSlot(),_sv$d;return (function(){var _b=_i;return EXTERN()&&_()&&(function(){ _val = {}; _val.scope = 'globals'; _val.structFwd = false; $at = _i;   /* the declaration itself - end-of-rule positions have skipped past it */ pendingProto = undefined; ; return true})()&&(function(){var _b=_i;return STRUCT()&&_()&&((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(function(){ _val.structFwd = true; $sname = $id; beginExternStruct($id, _s, _i); ; return true})()&&((function(){var _b=_i;return (_s[_i]==="{")&&(++_i,true)&&_()&&(function(){ /* a bodyless `extern struct G` never reaches endStruct, so only the braced form opens */ openStruct = $sname; ; return true})()&&((function(){while((function(){var _b=_i;return ((_sv$f=_val,_val=$f,((function(){var _b=_i;return ArrayDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||VarDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)})()))&&($f=_val,_val=_sv$f,true)||(_val=_sv$f,false))&&(function(){ addStructField($sname, { name: $f.name, type: $f.type, elem: $f.elem, struct: $f.struct, size: $f.size, dims: $f.dims }, _s, _i); ; return true})()&&((function(){var _b=_i;return (_s[_i]===";")&&(++_i,true)&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(_s[_i]==="}")&&(++_i,true)&&_()&&(function(){   /* Declare the host-owned interface so a signature-row consumer can check the layout the host supplies (.o.Name.field / .z.Name) against what Impala assumed, the way extern globals and function signatures are already checked. A re-declaration of a struct DEFINED here describes no host layout, so it publishes no row - the definition already published the real one. */ if (!endStruct($sname)) { emitStandaloneSignatureComment( structSignatureRow( $sname, true, sourceName, _s, declOffset)); } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(function(){var _b=_i;return FUNCTION()&&(function(){ _val.type  = 'U';  _val.scope = 'functions'; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||NATIVE()&&(function(){ _val.type  = 'N';  _val.scope = 'functions'; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&_()&&(function(){ $nmAt = _i; ; return true})()&&((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(function(){ _val.name  = $id; _val.at = $nmAt; ; return true})()&&((function(){var _b=_i;return (_s[_i]==="(")&&(++_i,true)&&_()&&((_sv$inp=_val,_val=$inp,(ArgsDecl()))&&($inp=_val,_val=_sv$inp,true)||(_val=_sv$inp,false))&&(_s[_i]===")")&&(++_i,true)&&_()&&(function(){   /* Optional PROTOTYPE. Name-only stays valid and asserts nothing (a wildcard a consumer skips); a prototype is a checkable assertion, so calls get argument type-checking and the emitted signature row carries real types. */ pendingProto = { args: copyParams($inp, $inp.n), ret: undefined }; ; return true})()&&((function(){var _b=_i;return RETURNS()&&_()&&((_sv$out=_val,_val=$out,(TypeDeclr()))&&($out=_val,_val=_sv$out,true)||(_val=_sv$out,false))&&(function(){   /* The name is OPTIONAL, as it already was for the sibling `functype ... returns V`: only type/elem/struct are read out of it, so `returns int` says everything `returns int n` did without making the author invent an identifier that is never printed nor referenced. TypeDeclr rather than a second rule for exactly that reason - the two declaration forms had no business differing. */ if (pendingProto) pendingProto.ret = { type: $out.type, elem: $out.elem, struct: $out.struct }; ; return true})()&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_()&&TypeDeclr()&&(function(){   /* PARKED for Impala 3.0 - see design/ParkedFeatures.md */ fail('Multiple return values are not supported in Impala 2.0', _s, _i, 'E428', 'declare a single return value for this extern'); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(function(){ externArray = true; ; return true})()&&((_sv$a=_val,_val=$a,(ArrayDecl()))&&($a=_val,_val=_sv$a,true)||(_val=_sv$a,false))&&(function(){ externArray = false; _val.type = 'A'; _val.name = $a.name; _val.at   = $a.at; _val.elem = $a.elem; _val.dims = $a.dims; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||(function(){ externArray = false; ; return true})()&&((_sv$d=_val,_val=$d,(VarDecl()))&&($d=_val,_val=_sv$d,true)||(_val=_sv$d,false))&&(function(){ _val.type = $d.type; _val.name = $d.name; _val.elem = $d.elem; _val.struct = $d.struct; _val.words = $d.words; _val.at = $d.at; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(function(){ if (_val.structFwd) { cancelStructRedeclaration($sname); return true; } declare( undefined,                 // no section for extern
                                                                             _val.scope, _val.name, _val.type, false,                     // not readonly
                                                                             '?', _s, _val.at, undefined, _val.elem ); /* A host-owned SHAPE needs an extent record even though every value in it belongs to the host: `dims` names the axes so a subscript can stride by `.d.g.0` and defer its bounds against it, and its LENGTH is the rank a subscript is checked against. Only for a shape - a rank-1 `extern array` keeps carrying no extent, exactly as before, so its every path is unchanged. `declare` rebuilds the record, so this has to land after it. */ if (_val.type === 'A' && _val.dims !== undefined) { symbols.globals[_val.name].extent = arrayExtent(_val.name, undefined, _val.name, undefined, _val.dims); } if (_val.scope === 'functions') { var entry = symbols.functions[_val.name]; var signature = entry && entry.signature; if (entry) { if (!signature) { signature = entry.signature = {}; } if (signature.sourceName === undefined) { signature.sourceName = sourceName; } if (signature.sourceCode === undefined) { signature.sourceCode = _s; signature.sourceOffset = declOffset; signature.sourceName = sourceName; } if (entry.kind !== 'FUNC') {  /* a definition here already resolved it - do not un-resolve it */ signature.returnResolved = false; } } var role = (_val.type === 'N' ? 'extern native' : 'extern func'); var placeholderSignature = { params: [], returns: undefined, sourceName: sourceName, sourceCode: _s, sourceOffset: declOffset, }; var _proto = pendingProto; pendingProto = undefined; if (_proto !== undefined) {   /* a declared prototype: real params + at most one return */ for (var _pi = 0; _pi < _proto.args.length; ++_pi) { var _p = _proto.args[_pi]; rejectByValueStruct(_p.type, _p.struct, _p.name, false, _s, $at); } var _pp = copyParams(_proto.args, _proto.args.length); var _pr = _proto.ret; if (_pr !== undefined) rejectByValueStruct(_pr.type, _pr.struct, _pr.name, true, _s, $at); placeholderSignature.params      = _pp; placeholderSignature.returns     = (_pr !== undefined ? _pr.type : 'V'); placeholderSignature.returnElem  = (_pr !== undefined ? _pr.elem : undefined); placeholderSignature.returnCount = (_pr !== undefined ? 1 : 0); placeholderSignature.returnWords = (_pr !== undefined ? 1 : 0); placeholderSignature.returnResolved = true; if (entry) { var _defined = (entry.kind === 'FUNC'); checkExternAgreement(_val.name, placeholderSignature, (_defined ? entry.signature : entry.externProto), _defined, _s, $at); entry.externProto = placeholderSignature; if (!_defined) {               /* a definition here outranks it; otherwise publish it so call sites check against it */ entry.signature.params         = _pp; entry.signature.returns        = placeholderSignature.returns; entry.signature.returnElem     = placeholderSignature.returnElem; entry.signature.returnCount    = placeholderSignature.returnCount; entry.signature.returnWords    = placeholderSignature.returnWords; entry.signature.returnResolved = true; } } } emitStandaloneSignatureComment( formatFunctionSignatureComment( _val.name, placeholderSignature, role, sourceName, _s, declOffset ) ); } else if (_val.scope === 'globals') { emitStandaloneSignatureComment( formatGlobalSignatureComment( 'GLOB', _val.name, _val.type, _val.size, 'extern', sourceName, _s, declOffset, _val.elem ) ); } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function ConstDecl(){var $base=newMetaSlot(),_sv$base,$desc,$nf,$t,$telem,$cStart,$id=newMetaSlot(),_sv$id,$cInitStart,$x=newMetaSlot(),_sv$x;return (function(){var _b=_i;return CONST()&&_()&&((_sv$base=_val,_val=$base,(TypeBase()))&&($base=_val,_val=_sv$base,true)||(_val=_sv$base,false))&&(function(){ /* Same type grammar as every other declarator (TypeBase, not bare BASE_TYPE), so a const can name a struct pointer or a named functype - a const is an assembler-level address/scalar constant, and those two are just addresses. A struct VALUE is the one shape that has no scalar constant form. */ $desc = $base; $nf = noForward; noForward = true; ; return true})()&&((function(){while((function(){var _b=_i;return POINTER()&&_()&&(function(){ $desc = pointerDesc($desc); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(function(){ var cd = descTypeElem($desc); if (cd.type === 'S') { fail('A const cannot be a struct value - use a struct pointer (' + 'const ' + cd.elem + ' pointer)', _s, _i, 'E447'); } $t = cd.type; $telem = cd.elem;    /* 't' + name for a funcptr constant */ $cStart = _i;   /* `Identifier` eats trailing space, so _i would name the NEXT declaration (E453) */ ; return true})()&&((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(function(){var _b=_i;return (_s[_i]==="=")&&(++_i,true)&&_()&&(function(){ $cInitStart = _i; ; return true})()&&((_sv$x=_val,_val=$x,(Expr()))&&($x=_val,_val=_sv$x,true)||(_val=_sv$x,false))&&(function(){ checkInitTarget($t, $telem, $x, _s, $cInitStart); declare( '! DEF?', 'defines', $id, $t, true, makeConstant($x, $t, _s, $cInitStart), _s, $cStart, formatConstSignatureComment( $id, $t, sourceName, _s, declOffset, $telem ), $telem ); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||(function(){ /* `export` says "this unit provides it"; a valueless const says "someone else does". The row below publishes it as an extern either way, so the keyword was silently dropped. */ if (exportNext) { fail('`export` contradicts a valueless `const` - ' + $id + ' is provided elsewhere, not by this unit', _s, $cStart, 'E453', 'give it a value to export it, or drop `export`'); } declare( undefined, 'defines', $id, $t, true, undefined, _s, $cStart, undefined, $telem ); emitStandaloneSignatureComment(  /* valueless -> host/runtime defines it: publish it as an extern so it links-checks */ formatConstSignatureComment( $id, $t, sourceName, _s, declOffset, $telem, true ) ); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(function(){ noForward = $nf; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function GlobalDecl(){var $section,$v=newMetaSlot(),_sv$v,$vStruct,$init,$initStart,$d=newMetaSlot(),_sv$d,$binit,$x=newMetaSlot(),_sv$x,$a=newMetaSlot(),_sv$a,$aStructEl,$shaped,$needNested;return (function(){var _b=_i;return (function(){var _b=_i;return GLOBAL()&&(function(){ $section = 'GLOB'; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||READONLY()&&(function(){ $section = 'CNST'; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||TEMPORARY()&&(function(){ $section = 'TEMP'; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&_()&&(function(){var _b=_i;return ((_sv$v=_val,_val=$v,(VarDecl()))&&($v=_val,_val=_sv$v,true)||(_val=_sv$v,false))&&(function(){ $vStruct = ($v.type === 'S'); if ($vStruct) {              /* struct value global -> one zeroed GLOB/CNST/TEMP *sizeof */ declare( $section, 'globals', $v.name, 'S', ($section === 'CNST'), structAllocSize($v.struct, _s, $v.at), _s, $v.at, formatGlobalSignatureComment( $section, $v.name, 'S', undefined, undefined, sourceName, _s, declOffset, $v.struct), structDesc($v.struct) ); } else { declare( $section, 'globals', undefined, $v.type, ($section === 'CNST'), '*1', _s, $v.at ); $init = ZEROES[$v.type]; } ; return true})()&&((function(){var _b=_i;return (_s[_i]==="=")&&(++_i,true)&&_()&&(function(){ $initStart = _i; ; return true})()&&(function(){var _b=_i;return ((_sv$d=_val,_val=$d,(Braced()))&&($d=_val,_val=_sv$d,true)||(_val=_sv$d,false))&&(function(){ if (!$vStruct) fail('Brace initializers are only for struct values', _s, $initStart, 'E422'); $binit = []; if (gazl2) {   /* SEEK-placed regions; see emitInitData */ $binit.seeks = []; $binit.path = []; } buildStructInit($v.struct, $d, $binit, _s, $initStart); emitInitData($binit, _s, $initStart); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||((_sv$x=_val,_val=$x,(Expr()))&&($x=_val,_val=_sv$x,true)||(_val=_sv$x,false))&&(function(){ if ($vStruct)                    /* $initStart, not _i: `Expr` ate the trailing space too */ fail('A struct value needs a brace initializer', _s, $initStart, 'E421'); checkInitTarget($v.type, $v.elem, $x, _s, $initStart); $init = makeConstant($x, $v.type, _s, $initStart); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&(function(){ if (!$vStruct) declare( 'DAT?', 'globals', $v.name, $v.type, ($section === 'CNST'), $init, _s, $v.at, formatGlobalSignatureComment( $section, $v.name, $v.type, undefined, undefined, sourceName, _s, declOffset, $v.elem ), $v.elem ); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||((_sv$a=_val,_val=$a,(ArrayDecl()))&&($a=_val,_val=_sv$a,true)||(_val=_sv$a,false))&&(function(){ declare( $section, 'globals', $a.name, 'A', ($section === 'CNST'), arrayAllocSize($a.elem, $a.size, $a.name, undefined, $a.dims), _s, $a.at, formatGlobalSignatureComment( $section, $a.name, 'A', $a.size, undefined, sourceName, _s, declOffset, $a.elem, $a.dims ), $a.elem ); symbols.globals[$a.name].extent = arrayExtent($a.name, undefined, $a.name, $a.size, $a.dims); returnExtent('global ' + $a.name,   /* declared: this consumer is done. Fields spelled out because bare `$a` is the rule's return value, not its context - see declEntry */ [ { size: $a.size, dims: $a.dims } ]); $aStructEl = isStructAtom($a.elem); ; return true})()&&((function(){var _b=_i;return (_s[_i]==="=")&&(++_i,true)&&_()&&(function(){   /* the list is fully consumed before these checks run, so _i would land on the NEXT declaration - name the initializer itself */ $initStart = _i; /* Hand InitList the DECLARED element type. Without it every entry was checked against its own type, which no value can fail, so `int array A[2] = { 1, "s" }` stored a pointer in an int slot and `float array F[2] = { 1, 2 }` stored the INTEGER bit pattern and read back 1.4013e-45. The scalar paths have always been this strict (`global float f = 1` is E407); only the array path was not. A struct-element array keeps reporting the friendlier E422 below, so it states no target at all. */ initTarget = ($aStructEl ? undefined : $a.elem); /* Every multi-dim shape mirrors its axes in the braces (`[2, 3]` -> `{ {,,}, {,,} }`), so a shape mistake is caught rather than silently shifting every element; a struct-element array nests one group per element the same way. Both need nested braces (a plain 1-D array stays flat). A literal shape is checked here; a symbolic one is validated at GAZL assembly - see buildShapedInit. */ $shaped = ($a.dims !== undefined); $needNested = ($shaped || $aStructEl); ; return true})()&&(function(){var _b=_i;return ((_sv$d=_val,_val=$d,(InitList()))&&($d=_val,_val=_sv$d,true)||(_val=_sv$d,false))&&(function(){   /* flat list -> 1-D scalar arrays and symbolic-axis shapes. `{ }` states nothing, so there is nothing to nest and nothing to place: it zero-fills, exactly as it already did for a struct value and a plain array. Rejecting it here asked for "one group per element" when no element had been given. */ if ($needNested && $d > 0) fail(($aStructEl ? 'A struct-element array needs nested braces, one group per element' : 'A shaped array needs nested braces, one group per axis'), _s, $initStart, 'E422'); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||((_sv$d=_val,_val=$d,(Braced()))&&($d=_val,_val=_sv$d,true)||(_val=_sv$d,false))&&(function(){   /* nested braces -> struct-element arrays and literal shapes */ if (!$needNested) fail('Nested brace initializers are for struct-element or shaped arrays', _s, $initStart, 'E422'); $binit = []; $binit.target = ($aStructEl ? undefined : $a.elem);   /* types the DATA rows; see emitInitData */ if ($aStructEl && gazl2) {   /* struct elements get SEEK-placed regions; a scalar shape's position is its index, layout-independent, so it stays positional even under --gazl2 */ $binit.seeks = []; $binit.path = []; } /* The same filler a struct FIELD uses. Nothing follows a standalone array inside its own allocation, so the gap it may report needs no blockInitFrom here. Fields spelled out because bare `$a` is the rule's return value, not its context - see declEntry. */ fillArray({ name: $a.name, elem: $a.elem, size: $a.size, dims: $a.dims }, undefined, $d, $binit, _s, $initStart); emitInitData($binit, _s, $initStart); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function Braced(){var $i=newMetaSlot(),_sv$i;return (function(){var _b=_i;return (_s[_i]==="{")&&(++_i,true)&&_()&&(function(){ _val = []; _val.n = 0; ; return true})()&&((function(){var _b=_i;return ((_sv$i=_val,_val=$i,(BracedEntry()))&&($i=_val,_val=_sv$i,true)||(_val=_sv$i,false))&&(function(){ _val[_val.n++] = $i; ; return true})()&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_()&&((_sv$i=_val,_val=$i,(BracedEntry()))&&($i=_val,_val=_sv$i,true)||(_val=_sv$i,false))&&(function(){ _val[_val.n++] = $i; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&(_s[_i]==="}")&&(++_i,true)&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function BracedEntry(){var $fname,$fat,$id=newMetaSlot(),_sv$id,$e=newMetaSlot(),_sv$e;return (function(){var _b=_i;return (function(){ $fname = undefined; $fat = _i; ; return true})()&&((function(){var _b=_i;return ((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(_s[_i]===":")&&(++_i,true)&&_()&&(function(){ $fname = $id; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&((_sv$e=_val,_val=$e,(BracedItem()))&&($e=_val,_val=_sv$e,true)||(_val=_sv$e,false))&&(function(){ var _e = $e;   /* bare `$e` is the VALUE; `$e.field` would set a CONTEXT property, and `Braced` stores only the value */ _e.field = $fname; _e.at = $fat; _val = _e; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function BracedItem(){var $b=newMetaSlot(),_sv$b,$x=newMetaSlot(),_sv$x;return (function(){var _b=_i;return ((_sv$b=_val,_val=$b,(Braced()))&&($b=_val,_val=_sv$b,true)||(_val=_sv$b,false))&&(function(){ _val = { braced: $b }; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||((_sv$x=_val,_val=$x,(Expr()))&&($x=_val,_val=_sv$x,true)||(_val=_sv$x,false))&&(function(){ var m = metaSlot($x); var op = makeRValue(m, '#<&'); if (span(op[0] || '', '#<&') !== 1) fail('Initializer must be a constant', _s, _i, 'E407'); _val = { op: holdConstant(op, m.type, _s, _i), type: m.type, elem: m.elem }; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function InitList(){var $d,$type,$n,$entryAt,$x=newMetaSlot(),_sv$x;return (function(){var _b=_i;return (_s[_i]==="{")&&(++_i,true)&&_()&&(function(){ $d = ' '; $type = undefined; $n = 0; ; return true})()&&((function(){var _b=_i;return (function(){var _l=_i,_lv=_val,_x=(function(){var _b=_i;return Identifier()&&(_s[_i]===":")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)})();_i=_l;_val=_lv;return !_x})()&&(function(){ $entryAt = _i; ; return true})()&&((_sv$x=_val,_val=$x,(Expr()))&&($x=_val,_val=_sv$x,true)||(_val=_sv$x,false))&&(function(){ var first = initEntry(metaSlot($x), _s, $entryAt); $type = first.type; $d += first.op; ++$n; ; return true})()&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_()&&(function(){var _l=_i,_lv=_val,_x=(function(){var _b=_i;return Identifier()&&(_s[_i]===":")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)})();_i=_l;_val=_lv;return !_x})()&&(function(){ $entryAt = _i; ; return true})()&&((_sv$x=_val,_val=$x,(Expr()))&&($x=_val,_val=_sv$x,true)||(_val=_sv$x,false))&&(function(){ var next = initEntry(metaSlot($x), _s, $entryAt); var xType = next.type, constant = next.op; /* decide if we need to flush DATA */ if (  constant[0] === '<' || $d[1] === '<' || ($d + ' ' + constant).length >= 55) { declare( initElemRow(), 'globals', undefined, xType, true, $d.substr(1), _s, _i ); $d = ''; } $d += ' ' + constant; $type = xType; ++$n; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&(_s[_i]==="}")&&(++_i,true)&&_()&&(function(){ _val = $n;   /* entry count. (Set at the END historically because the `!(Identifier ':')` predicate compiles to `Identifier()` and used to clobber `_val` mid-rule; predicates now restore it.) */ if ($d.substr(1) !== '') { declare( initElemRow(), 'globals', undefined, $type, true, $d.substr(1), _s, _i ); } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function ArgsDecl(){var $v=newMetaSlot(),_sv$v;return (function(){var _b=_i;return (function(){ _val = []; _val.n = 0; ; return true})()&&((function(){var _b=_i;return ((_sv$v=_val,_val=$v,(VarDecl()))&&($v=_val,_val=_sv$v,true)||(_val=_sv$v,false))&&(function(){ _val[_val.n++] = declEntry($v.type, $v.elem, $v.struct, $v.words, $v.name, $v.size, $v.dims, $v.at); ; return true})()&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_()&&((_sv$v=_val,_val=$v,(VarDecl()))&&($v=_val,_val=_sv$v,true)||(_val=_sv$v,false))&&(function(){ _val[_val.n++] = declEntry($v.type, $v.elem, $v.struct, $v.words, $v.name, $v.size, $v.dims, $v.at); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function LocalsDecl(){var $v=newMetaSlot(),_sv$v;return (function(){var _b=_i;return (function(){ _val = []; _val.n = 0; ; return true})()&&((function(){var _b=_i;return ((_sv$v=_val,_val=$v,((function(){var _b=_i;return VarDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||ArrayDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)})()))&&($v=_val,_val=_sv$v,true)||(_val=_sv$v,false))&&(function(){ _val[_val.n++] = declEntry($v.type, $v.elem, $v.struct, $v.words, $v.name, $v.size, $v.dims, $v.at); ; return true})()&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_()&&((_sv$v=_val,_val=$v,((function(){var _b=_i;return VarDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)||ArrayDecl()||(_im=(_i>_im?_i:_im),_i=_b,false)})()))&&($v=_val,_val=_sv$v,true)||(_val=_sv$v,false))&&(function(){ _val[_val.n++] = declEntry($v.type, $v.elem, $v.struct, $v.words, $v.name, $v.size, $v.dims, $v.at); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function TypeBase(){var $t=newMetaSlot(),_sv$t,$id=newMetaSlot(),_sv$id;return (function(){var _b=_i;return ((_sv$t=_val,_val=$t,(BASE_TYPE()))&&($t=_val,_val=_sv$t,true)||(_val=_sv$t,false))&&_()&&(function(){ _val = CASTS_TO_TYPES[$t]; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(function(){ /* the ONE place a raw type name becomes a descriptor */ if (isStructName($id)) _val = structDesc($id); else if (isFuncTypeName($id)) _val = funcTypeDesc($id); else fail('Unknown type ' + $id, _s, _i, 'E413'); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function VarDecl(){var $base=newMetaSlot(),_sv$base,$desc,$nameStart,$id=newMetaSlot(),_sv$id;return (function(){var _b=_i;return ((_sv$base=_val,_val=$base,(TypeBase()))&&($base=_val,_val=_sv$base,true)||(_val=_sv$base,false))&&(function(){ _val = {}; $desc = $base; ; return true})()&&((function(){while((function(){var _b=_i;return POINTER()&&_()&&(function(){ $desc = pointerDesc($desc); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(function(){ $nameStart = _i; ; return true})()&&((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(function(){ checkReservedName($id, 'variable', _s, $nameStart); var vd = descTypeElem($desc); if (vd.type === 'S') { _val.type = 'S'; _val.struct = vd.elem; _val.elem = undefined; _val.words = structWords(vd.elem); } else { _val.type = vd.type;       /* 't' + the name for a named funcptr type */ _val.elem = vd.elem; _val.struct = undefined; _val.words = undefined;    /* scalar: a single word, no size operand */ } _val.name = $id; _val.at   = $nameStart;   /* the NAME - `Identifier` ate the trailing space, so an end-of-rule _i would name the NEXT declaration */ _val.size = undefined; _val.dims = undefined;    /* a recycled slot: a scalar after a SHAPED array would inherit its axes */ ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function ArrayDecl(){var $desc,$extent,$dims,$base=newMetaSlot(),_sv$base,$nameStart,$id=newMetaSlot(),_sv$id,$extentStart,$x=newMetaSlot(),_sv$x,$y=newMetaSlot(),_sv$y,$size;return (function(){var _b=_i;return (function(){ _val = {}; $desc = undefined; $extent = undefined; $dims = undefined; ; return true})()&&((function(){var _b=_i;return ((_sv$base=_val,_val=$base,(TypeBase()))&&($base=_val,_val=_sv$base,true)||(_val=_sv$base,false))&&(function(){ $desc = $base; ; return true})()&&((function(){while((function(){var _b=_i;return POINTER()&&_()&&(function(){ $desc = pointerDesc($desc); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&ARRAY()&&_()&&(function(){ $nameStart = _i; ; return true})()&&((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(function(){ checkReservedName($id, 'array', _s, $nameStart); ; return true})()&&((function(){var _b=_i;return (_s[_i]==="[")&&(++_i,true)&&_()&&(function(){ $extentStart = _i; $dims = []; ; return true})()&&(function(){var _b=_i;return ((_sv$x=_val,_val=$x,(Expr()))&&($x=_val,_val=_sv$x,true)||(_val=_sv$x,false))&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_()&&((_sv$y=_val,_val=$y,(Expr()))&&($y=_val,_val=_sv$y,true)||(_val=_sv$y,false))&&(function(){   /* an EXTRA axis. Held as its own constant; the axes stay separate so each can be checked against its own index, and only the PRODUCT becomes the allocation size. */ $dims.push(arrayAxis($y, _s, $extentStart)); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(_s[_i]==="]")&&(++_i,true)&&_()&&(function(){ $size = makeConstant($x, 'i', _s, $extentStart); /* A NEGATIVE extent runs the layout BACKWARDS. On a plain array the assembler catches it, because the count reaches it as a `GLOB *size` / `LOCA *size` operand it type-checks ("Incompatible types: .z.g"). A struct FIELD's count is only ever ADDED into the offset accumulator, which subtracts without complaint - `struct T { int a; int array b[-1]; int c }` compiles, assembles, runs, and puts `a` and `c` in the SAME WORD. Rejected here, at the declaration, because that is where the extent is still a number and where the invariant belongs. Zero is left alone: it wastes a field but aliases nothing, and every constant index into it is already out of range. */ if (constInt($size) < 0) fail('Array extent is negative: ' + dropHash($size), _s, $extentStart, 'E462', 'an array holds zero or more elements'); $extent = dropHash($size);   /* element count - may be a symbolic const */ if ($dims.length > 0) {              /* a SHAPE: allocate the product, keep the axes */ $dims.unshift($extent);          /* written order, outermost first */ $extent = axesProduct($dims); $dims.reverse();                 /* stride order: axis 0 is innermost */ } /* THE CONSUMER OWNS THE BORROW. A folded extent lives in a `<X>` scratch, and a scratch is recycled on the next borrow - so whoever still has to READ this one decides when it goes back: endStruct after the layout block, FuncDecl after the locals pass, GlobalDecl right after its declaration. Freeing it here instead let the NEXT declarator in the same list borrow the same scratch and overwrite the extent, which is how two array locals in one clause silently got whichever count was folded last. */ ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_()&&(function(){ $dims.push(undefined); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(_s[_i]==="]")&&(++_i,true)&&_()&&(function(){ $dims.push(undefined);   /* rank = commas + 1, so `[]` is rank 1 */ ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&(function(){ /* A HOST-OWNED array states its RANK and no extent: the host supplies `.o.`, `.z.` and every `.d.`, so a number here would be an unverifiable claim Impala never reads. Rank is not optional with it, because rank is what a subscript is checked against and what a stride is read from - and it is the ONLY thing about the layout this side genuinely knows. */ var hostOwned = hostOwnedArray(); if (hostOwned && $extent !== undefined) { fail('A host-owned array must not state a size', _s, $extentStart, 'E430', 'the host owns this layout - state only the rank, `array ' + $id + '[' + new Array($dims.length).join(',') + ']`');   /* rank N reads as N-1 commas */ } if (hostOwned && $dims === undefined) { fail('Host-owned array ' + $id + ' must state its rank', _s, $nameStart, 'E432', 'write `array ' + $id + '[]` for one axis, `[,]` for two - ' + 'one comma per axis after the first'); } if (!hostOwned && $extent === undefined) { fail('Array ' + $id + ' needs a size', _s, $nameStart, 'E431', 'only a host-owned array - an `extern array` or an `extern ' + 'struct` field - states rank without extents'); } _val.type = 'A'; _val.elem = $desc; _val.name = $id; _val.at   = $nameStart;                 /* see VarDecl */ _val.size = $extent; _val.words = $extent;                   /* an array's words ARE its count; a struct element scales symbolically, in arrayAllocSize */ _val.dims = (($dims !== undefined && $dims.length > 1) ? $dims : undefined); /* only a SHAPE carries axes; a 1-D array has none, which is what keeps its every path unchanged */ ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function Statement(){var $lblStart,$label=newMetaSlot(),_sv$label;return (function(){var _b=_i;return (function(){ /* Scanned IN PLACE rather than copying the rest of the unit first. `cut >= 0` was dead: find returns 0..length and never negative. (Measured: the copy cost nothing - V8 slices rather than copies - so this is a dead-branch removal, NOT the perf fix it looks like.) */ var cut = find(_s, "{;\r\n", _i); var txt = _s.substring(_i, cut); emitMeta({ operator:';', type:undefined, operands:[ txt, undefined, undefined ] }); ; return true})()&&((function(){while((function(){var _b=_i;return (function(){ $lblStart = _i; ; return true})()&&((_sv$label=_val,_val=$label,(Identifier()))&&($label=_val,_val=_sv$label,true)||(_val=_sv$label,false))&&(_s[_i]===":")&&(++_i,true)&&_()&&(function(){ emitMeta({ operator:'<--', type:undefined, operands:[ '@' + $label, undefined, undefined ] }); /* carry the source position so processBranches can name the `.impala` line if this label is a duplicate */ var lbl = metacode[metacode.length - 1]; lbl.labelSource = _s; lbl.labelOffset = _i; /* the 1.x `goto break;` early-exit idiom is the --legacy case */ checkReservedName($label, 'label', _s, $lblStart); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(function(){var _b=_i;return (_s[_i]===";")&&(++_i,true)&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)||Assert()||(_im=(_i>_im?_i:_im),_i=_b,false)||Block()||(_im=(_i>_im?_i:_im),_i=_b,false)||Copy()||(_im=(_i>_im?_i:_im),_i=_b,false)||DoWhile()||(_im=(_i>_im?_i:_im),_i=_b,false)||Loop()||(_im=(_i>_im?_i:_im),_i=_b,false)||For()||(_im=(_i>_im?_i:_im),_i=_b,false)||Goto()||(_im=(_i>_im?_i:_im),_i=_b,false)||Return()||(_im=(_i>_im?_i:_im),_i=_b,false)||Break()||(_im=(_i>_im?_i:_im),_i=_b,false)||Continue()||(_im=(_i>_im?_i:_im),_i=_b,false)||If()||(_im=(_i>_im?_i:_im),_i=_b,false)||Switch()||(_im=(_i>_im?_i:_im),_i=_b,false)||While()||(_im=(_i>_im?_i:_im),_i=_b,false)||Destructure()||(_im=(_i>_im?_i:_im),_i=_b,false)||Tail()||(_im=(_i>_im?_i:_im),_i=_b,false)||Expr()&&(_s[_i]===";")&&(++_i,true)&&_()&&(function(){ releaseMeta(_val); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function Expr(){var $r=newMetaSlot(),_sv$r;return (function(){var _b=_i;return Bitwise()&&((function(){var _b=_i;return (_s[_i]==="=")&&(++_i,true)&&_()&&((_sv$r=_val,_val=$r,(Expr()))&&($r=_val,_val=_sv$r,true)||(_val=_sv$r,false))&&(function(){ if (!dry) assign(_val, _val, $r, _s, _i); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function Bitwise(){var $first,$op=newMetaSlot(),_sv$op,$r=newMetaSlot(),_sv$r;return (function(){var _b=_i;return (function(){ $first = undefined; ; return true})()&&AddSub()&&((function(){while((function(){var _b=_i;return ((_sv$op=_val,_val=$op,(BITWISE_OP()))&&($op=_val,_val=_sv$op,true)||(_val=_sv$op,false))&&_()&&((_sv$r=_val,_val=$r,(AddSub()))&&($r=_val,_val=_sv$r,true)||(_val=_sv$r,false))&&(function(){ if (!dry) { if ($first === undefined) $first = $op; else if ($first !== $op) mixedBitwise($first, $op, _s, _i); binaryOp($op, _val, $r, _s, _i); } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(function(){ if (!dry) stampBitwise(_val, $first !== undefined); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function AddSub(){var $op=newMetaSlot(),_sv$op,$r=newMetaSlot(),_sv$r;return (function(){var _b=_i;return MulDiv()&&((function(){while((function(){var _b=_i;return ((_sv$op=_val,_val=$op,(ADDSUB_OP()))&&($op=_val,_val=_sv$op,true)||(_val=_sv$op,false))&&_()&&((_sv$r=_val,_val=$r,(MulDiv()))&&($r=_val,_val=_sv$r,true)||(_val=_sv$r,false))&&(function(){ if (!dry) binaryOp($op, _val, $r, _s, _i); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function MulDiv(){var $op=newMetaSlot(),_sv$op,$r=newMetaSlot(),_sv$r;return (function(){var _b=_i;return PrePost()&&((function(){while((function(){var _b=_i;return ((_sv$op=_val,_val=$op,(MULDIV_OP()))&&($op=_val,_val=_sv$op,true)||(_val=_sv$op,false))&&_()&&((_sv$r=_val,_val=$r,(PrePost()))&&($r=_val,_val=_sv$r,true)||(_val=_sv$r,false))&&(function(){ if (!dry) mulDivOp($op, _val, $r, _s, _i); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function PrePost(){var $op=newMetaSlot(),_sv$op,$cdesc,$ccast,$sid=newMetaSlot(),_sv$sid,$pdepth;return (function(){var _b=_i;return (function(){var _b=_i;return ((_sv$op=_val,_val=$op,(PREFIX_OP()))&&($op=_val,_val=_sv$op,true)||(_val=_sv$op,false))&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="(")&&(++_i,true)&&_()&&((_sv$op=_val,_val=$op,(BASE_TYPE()))&&($op=_val,_val=_sv$op,true)||(_val=_sv$op,false))&&_()&&(function(){ $cdesc = CASTS_TO_TYPES[$op]; ; return true})()&&((function(){while((function(){var _b=_i;return POINTER()&&_()&&(function(){ $cdesc = pointerDesc($cdesc); $ccast = 'pointer'; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(_s[_i]===")")&&(++_i,true)&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="(")&&(++_i,true)&&_()&&((_sv$sid=_val,_val=$sid,(Identifier()))&&($sid=_val,_val=_sv$sid,true)||(_val=_sv$sid,false))&&(function(){ $pdepth = 0; ; return true})()&&((function(){while((function(){var _b=_i;return POINTER()&&_()&&(function(){ $pdepth = $pdepth + 1; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(_s[_i]===")")&&(++_i,true)&&_()&&(function(){ /* With no `pointer` this is only a cast when the name is a FUNCTYPE, which is already a pointer and so needs no modifier - a struct has no by-value cast, and anything else is a plain parenthesized expression. Backing out (rather than failing) lets Value parse `(x) + 1`; the test above is a pure lookup, so there is no side effect to undo. */ if ($pdepth === 0) { if (!isFuncTypeName($sid)) return false; $cdesc = funcTypeDesc($sid); $ccast = 'funcptr'; } else { if (!isStructName($sid) && !isFuncTypeName($sid)) fail('Unknown type ' + $sid, _s, _i, 'E413'); $cdesc = (isStructName($sid) ? structDesc($sid) : funcTypeDesc($sid)); for (var _pk = 0; _pk < $pdepth; ++_pk) $cdesc = pointerDesc($cdesc); $ccast = 'pointer'; } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&PrePost()&&(function(){ if (!dry) { if ($ccast) {          /* a named/pointer cast; a bare BASE_TYPE cast has no $ccast */ if ($ccast === 'funcptr') { checkFuncTypeCast(_val, descTail($cdesc), _s, _i); } unaryOp($ccast, _val, _s, _i); setElem(_val, descTail($cdesc)); } else { unaryOp($op, _val, _s, _i); } } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||Value()&&((function(){while((function(){var _b=_i;return FuncCall()||(_im=(_i>_im?_i:_im),_i=_b,false)||Subscript()||(_im=(_i>_im?_i:_im),_i=_b,false)||FieldAccess()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
...
function Comp(){var $op=newMetaSlot(),_sv$op,$r=newMetaSlot(),_sv$r;return (function(){var _b=_i;return (_s[_i]==="!")&&(++_i,true)&&_()&&(function(){ /* `!` sits BELOW comparison, so `!x == 2` means `!(x == 2)` - the opposite of the C reading. Require its operand to be parenthesised (or another `!`); reuse the strict-expression machinery so `--legacy` keeps the old meaning with a warning. */ var c = _s.charAt(_i); if (c !== '!' && c !== '(') strictError( "'!' binds below comparison; parenthesise its operand", _s, _i, 'E103', 'write !(a == b), or to negate a value use (a) != b'); ; return true})()&&Comp()&&(function(){ emit('!', undefined, undefined, undefined, undefined); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||(function(){ dry = true; ; return true})()&&(function(){var _l=_i,_lv=_val,_x=Group();_i=_l;_val=_lv;return !_x})()&&(function(){ dry = false; ; return true})()&&BoolGroup()||(_im=(_i>_im?_i:_im),_i=_b,false)||(function(){ dry = false; ; return true})()&&Expr()&&((_sv$op=_val,_val=$op,(COMP_OP()))&&($op=_val,_val=_sv$op,true)||(_val=_sv$op,false))&&_()&&((_sv$r=_val,_val=$r,(Expr()))&&($r=_val,_val=_sv$r,true)||(_val=_sv$r,false))&&(function(){ checkCompMix(_val, $r, _s, _i); binaryOp($op, _val, $r, _s, _i); emitMeta(_val); releaseMeta(_val); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function Assert(){var $okLabel,$start,$exprText,$str=newMetaSlot(),_sv$str,$;return (function(){var _b=_i;return ASSERT()&&_()&&(function(){ $okLabel  = beginDebugGuard('a'); $start    = _i; $exprText = ''; ; return true})()&&((_sv$str=_val,_val=$str,(BoolGroup()))&&($str=_val,_val=_sv$str,true)||(_val=_sv$str,false))&&(function(){   /* the assert's own source text becomes its message */ $exprText = _s.substring($start, _i); $exprText = $exprText.replace(/[ \t\r\n]+$/, ''); ; return true})()&&(_s[_i]===";")&&(++_i,true)&&_()&&(function(){ endDebugGuard($okLabel, $str, $exprText, _s, _i); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
...
function Goto(){var $gotoStart,$label=newMetaSlot(),_sv$label;return (function(){var _b=_i;return GOTO()&&_()&&(function(){ $gotoStart = _i; ; return true})()&&((_sv$label=_val,_val=$label,(Identifier()))&&($label=_val,_val=_sv$label,true)||(_val=_sv$label,false))&&(_s[_i]===";")&&(++_i,true)&&_()&&(function(){ emit('-->', undefined, '@' + $label, undefined, undefined); /* carry the source position so processBranches can name the `.impala` line if this label is never defined (E445) */ var g = metacode[metacode.length - 1]; g.gotoSource = _s; g.gotoOffset = $gotoStart; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
...
function Tail(){var $tailAt,$id=newMetaSlot(),_sv$id;return (function(){var _b=_i;return TAIL()&&_()&&(function(){ $tailAt = _i; ; return true})()&&((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(function(){ if (_s.charAt(_i) !== '(') return false;   /* pure lookahead: nothing to undo */ if (inlineMode) { fail('tail cannot be used in an inline function', _s, $tailAt, 'E468', 'an inline body expands into its caller and runs in that frame, so there is no frame of its own to reuse'); } if (!gazl2) { fail('tail requires --gazl2', _s, $tailAt, 'E466', 'the TAIL instruction it compiles to exists only on GAZL 2 engines'); } if ($id !== tailFn) { fail('tail can only target the enclosing function in Impala 2.0 (' + $id + ' is not ' + tailFn + ')', _s, $tailAt, 'E467', 'the window and return contracts of a cross-function tail are not checked yet - call it, or restructure into self-recursion'); } lookup(_val, $id, false, _s, $tailAt); metaSlot(_val).tailCall = true; ; return true})()&&FuncCall()&&(_s[_i]===";")&&(++_i,true)&&_()&&(function(){ releaseMeta(_val); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function Break(){var $start;return (function(){var _b=_i;return (function(){ $start = _i; ; return true})()&&(_s.substr(_i,5)==="break")&&(_i+=5,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()&&_()&&(_s[_i]===";")&&(++_i,true)&&_()&&(function(){ fail("'break' is not supported: a switch arm already does not fall through, and a loop is left with `goto` to a label after it", _s, $start, 'E450', 'replace `break;` with `goto` to a label placed after the loop'); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function Continue(){var $start;return (function(){var _b=_i;return (function(){ $start = _i; ; return true})()&&(_s.substr(_i,8)==="continue")&&(_i+=8,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()&&_()&&(_s[_i]===";")&&(++_i,true)&&_()&&(function(){ fail("'continue' is not supported: jump to a label at the end of the loop body with `goto`", _s, $start, 'E450', 'replace `continue;` with `goto` to a label at the loop-body end'); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
...
function For(){var $varStart,$var=newMetaSlot(),_sv$var,$gotInit,$init=newMetaSlot(),_sv$init,$toExpr=newMetaSlot(),_sv$toExpr,$type,$to,$noLoopLabel,$loopLabel,$body=newMetaSlot(),_sv$body;return (function(){var _b=_i;return FOR()&&_()&&(_s[_i]==="(")&&(++_i,true)&&_()&&(function(){ $varStart = _i; ; return true})()&&((_sv$var=_val,_val=$var,(Variable()))&&($var=_val,_val=_sv$var,true)||(_val=_sv$var,false))&&(function(){ /* loop-variable must be local, modifiable int / pointer */ var varMeta = metaSlot($var); if (varMeta.operator !== '=' || span(varMeta.type, "ip") === 0) { fail( 'For variable must be a local modifiable int or pointer variable', _s, $varStart , 'E305', 'a parameter, global or non-scalar cannot be the loop variable - copy it into a `locals` int or pointer and loop over that'); } /* `FORp` steps exactly one WORD and is already a 3-operand form, so a struct pointer has nowhere to put its stride. Scaling only the bound silently ran sizeof(S) times too many (F2 in design/impala/Impala2Review.md). */ if (strideStruct(varMeta.elem) !== undefined) { fail( 'For variable must not be a struct pointer', _s, _i, 'E309', 'FORp cannot stride by a struct - use `while (p < end) { ...; p = &p[1]; }`'); } $gotInit = false;            /* flag to detect an explicit start value */ ; return true})()&&((function(){var _b=_i;return (_s[_i]==="=")&&(++_i,true)&&_()&&((_sv$init=_val,_val=$init,(Expr()))&&($init=_val,_val=_sv$init,true)||(_val=_sv$init,false))&&(function(){ assign($init, $var, $init, _s, _i); $gotInit = true; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&TO()&&_()&&((_sv$toExpr=_val,_val=$toExpr,(Expr()))&&($toExpr=_val,_val=_sv$toExpr,true)||(_val=_sv$toExpr,false))&&(_s[_i]===")")&&(++_i,true)&&_()&&(function(){ var varMeta  = metaSlot($var); var toMeta   = metaSlot($toExpr); if (toMeta.type !== varMeta.type) { typeError( 'Incompatible types ({$type1} and {$type2})', _s, _i, varMeta.type, toMeta.type , 'E301'); } /* constant upper bound */ $to = makeRValue(toMeta); /* initial comparison  (var < to)                         */ emit( '<', toMeta.type, undefined, $gotInit ? metaSlot($init).operands[1]     /* start value from “var = expr” */ : varMeta.operands[1],            /* or the original variable */ $to ); if ($gotInit) { releaseMeta($init); } /* branch-out   and  loop label */ $noLoopLabel = newLabel('e'); emit('?->', false, $noLoopLabel, undefined, undefined); $loopLabel   = newLabel('l'); emit('<--', undefined, $loopLabel, undefined, undefined); ; return true})()&&((_sv$body=_val,_val=$body,(Statement()))&&($body=_val,_val=_sv$body,true)||(_val=_sv$body,false))&&(function(){ var varMeta = metaSlot($var); /* increment + jump back. A struct pointer never reaches here - E309 above rejects it, because `FORp` is already a 3-operand form with nowhere to put a stride (F2 in design/impala/Impala2Review.md). */ emit( '...', varMeta.type, varMeta.operands[1],        /* address of loop variable */ $to, $loopLabel ); emit('<-?', false, $noLoopLabel, undefined, undefined); returnBack($to); releaseMeta(varMeta); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function Copy(){var $l=newMetaSlot(),_sv$l,$f=newMetaSlot(),_sv$f,$t=newMetaSlot(),_sv$t,$length,$type,$;return (function(){var _b=_i;return COPY()&&_()&&(_s[_i]==="(")&&(++_i,true)&&_()&&((_sv$l=_val,_val=$l,(Expr()))&&($l=_val,_val=_sv$l,true)||(_val=_sv$l,false))&&FROM()&&_()&&((_sv$f=_val,_val=$f,(Expr()))&&($f=_val,_val=_sv$f,true)||(_val=_sv$f,false))&&TO()&&_()&&((_sv$t=_val,_val=$t,(Expr()))&&($t=_val,_val=_sv$t,true)||(_val=_sv$t,false))&&(_s[_i]===")")&&(++_i,true)&&_()&&(function(){ var fromMeta = metaSlot($f); var toMeta   = metaSlot($t); $length = makeConstant($l, 'i', _s, _i); var lengthHash = dropHash($length); if (fromMeta.type + toMeta.type !== 'pp') { returnBack($length); typeError( 'Invalid types ({$type1} and {$type2})', _s, _i, fromMeta.type, toMeta.type , 'E301'); } /* Both are pointers; ask what they point AT - but only when BOTH sides know, because `copy` flags a CONTRADICTION, not a broken promise. An assignment rejects untyped -> typed (E201): the variable must keep that promise for every later deref. A copy consumes both addresses on the spot, so an untyped source claims nothing to break - and reading an Impala 1 `array` blob into typed storage is the 1.0 idiom, not a defect. The LENGTH stays unchecked on purpose: a pointer has no extent. */ if (toMeta.elem !== undefined && fromMeta.elem !== undefined) { checkPtrAssign(toMeta, fromMeta, _s, _i); } var copyMeta = metaSlot($l); makeMeta( copyMeta, 'copy', '?', makeRValue(toMeta, '&$%'), makeRValue(fromMeta, '&$%'), '*' + lengthHash ); emitMeta(copyMeta); returnBack($length); releaseMeta(copyMeta); ; return true})()&&(_s[_i]===";")&&(++_i,true)&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
...
function DestTarget(){var $tgtGlobal,$id=newMetaSlot(),_sv$id,$tgtName;return (function(){var _b=_i;return (function(){ $tgtGlobal = false; ; return true})()&&((function(){var _b=_i;return GLOBAL()&&_()&&(function(){ $tgtGlobal = true; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)&&((_sv$id=_val,_val=$id,(Identifier()))&&($id=_val,_val=_sv$id,true)||(_val=_sv$id,false))&&(function(){ $tgtName = $id; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
...
function Identifier(){return (function(){var _b=_i;return (function(){var _l=_i,_lv=_val,_x=KEYWORD();_i=_l;_val=_lv;return !_x})()&&(function(){var _m=_i;return (function(){var _b=_i;return (!!_s[_i]&&"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_$".indexOf(_s[_i])>=0)&&(++_i,true)&&((function(){while(SYMBOL_CHAR());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(_val=_s.slice(_m,_i),true)})()&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
...
function IntegerLiteral(){return (function(){var _b=_i;return (function(){var _m=_i;return (function(){var _b=_i;return ((!!_s[_i]&&"-+".indexOf(_s[_i])>=0)&&(++_i,true),true)&&(function(){var _b=_i;return (_s.substr(_i,2)==="0x")&&(_i+=2,true)&&((function(){for(var _n=0;HEX();++_n);return _n>0})())||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="'")&&(++_i,true)&&((function(){while((function(){var _b=_i;return (function(){var _l=_i,_lv=_val,_x=(_s[_i]==="'")&&(++_i,true);_i=_l;_val=_lv;return !_x})()&&ASCII()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(_s[_i]==="'")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||((function(){for(var _n=0;DIGIT();++_n);return _n>0})())||(_im=(_i>_im?_i:_im),_i=_b,false)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(_val=_s.slice(_m,_i),true)})()&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function StringLiteral(){return (function(){var _b=_i;return (function(){var _m=_i;return (function(){var _b=_i;return (_s[_i]==="\"")&&(++_i,true)&&((function(){while((function(){var _b=_i;return (function(){var _l=_i,_lv=_val,_x=(!!_s[_i]&&"\"\\\u0000\u0001\u0002\u0003\u0004\u0005\u0006\u0007\b\t\n\u000b\f\r\u000e\u000f\u0010\u0011\u0012\u0013\u0014\u0015\u0016\u0017\u0018\u0019\u001a\u001b\u001c\u001d\u001e\u001f ¡¢£¤¥¦§¨©ª«¬­®¯°±²³´µ¶·¸¹º»¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ".indexOf(_s[_i])>=0)&&(++_i,true);_i=_l;_val=_lv;return !_x})()&&(!!_s[_i])&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="\\")&&(++_i,true)&&(function(){var _b=_i;return (!!_s[_i]&&"\"\\bfnrt".indexOf(_s[_i])>=0)&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="u")&&(++_i,true)&&HEX()&&HEX()&&HEX()&&HEX()||(_im=(_i>_im?_i:_im),_i=_b,false)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(_s[_i]==="\"")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(_val=_s.slice(_m,_i),true)})()&&_()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
...
function ABS(){return (function(){var _b=_i;return (_s.substr(_i,3)==="abs")&&(_i+=3,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function ARRAY(){return (function(){var _b=_i;return (_s.substr(_i,5)==="array")&&(_i+=5,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function ASSERT(){return (function(){var _b=_i;return (_s.substr(_i,6)==="assert")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function CASE(){return (function(){var _b=_i;return (_s.substr(_i,4)==="case")&&(_i+=4,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function CONST(){return (function(){var _b=_i;return (_s.substr(_i,5)==="const")&&(_i+=5,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function COPY(){return (function(){var _b=_i;return (_s.substr(_i,4)==="copy")&&(_i+=4,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function DEFAULT(){return (function(){var _b=_i;return (_s.substr(_i,7)==="default")&&(_i+=7,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function DO(){return (function(){var _b=_i;return (_s.substr(_i,2)==="do")&&(_i+=2,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function ELSE(){return (function(){var _b=_i;return (_s.substr(_i,4)==="else")&&(_i+=4,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function EXPORT(){return (function(){var _b=_i;return (_s.substr(_i,6)==="export")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function IMPORT(){return (function(){var _b=_i;return (_s.substr(_i,6)==="import")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function EXTERN(){return (function(){var _b=_i;return (_s.substr(_i,6)==="extern")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function FLOAT(){return (function(){var _b=_i;return (_s.substr(_i,5)==="float")&&(_i+=5,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function INLINE(){return (function(){var _b=_i;return (_s.substr(_i,6)==="inline")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function FLOOR(){return (function(){var _b=_i;return (_s.substr(_i,5)==="floor")&&(_i+=5,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function FOR(){return (function(){var _b=_i;return (_s.substr(_i,3)==="for")&&(_i+=3,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function FROM(){return (function(){var _b=_i;return (_s.substr(_i,4)==="from")&&(_i+=4,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function FTOI(){return (function(){var _b=_i;return (_s.substr(_i,4)==="ftoi")&&(_i+=4,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function FUNCPTR(){return (function(){var _b=_i;return (_s.substr(_i,7)==="funcptr")&&(_i+=7,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function FUNCTYPE(){return (function(){var _b=_i;return (_s.substr(_i,8)==="functype")&&(_i+=8,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function FUNCTION(){return (function(){var _b=_i;return (_s.substr(_i,8)==="function")&&(_i+=8,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function GLOBAL(){return (function(){var _b=_i;return (_s.substr(_i,6)==="global")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function GOTO(){return (function(){var _b=_i;return (_s.substr(_i,4)==="goto")&&(_i+=4,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function IF(){return (function(){var _b=_i;return (_s.substr(_i,2)==="if")&&(_i+=2,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function INT(){return (function(){var _b=_i;return (_s.substr(_i,3)==="int")&&(_i+=3,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function ITOF(){return (function(){var _b=_i;return (_s.substr(_i,4)==="itof")&&(_i+=4,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function LOCALS(){return (function(){var _b=_i;return (_s.substr(_i,6)==="locals")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function LOOP(){return (function(){var _b=_i;return (_s.substr(_i,4)==="loop")&&(_i+=4,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function NATIVE(){return (function(){var _b=_i;return (_s.substr(_i,6)==="native")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function NULL(){return (function(){var _b=_i;return (_s.substr(_i,4)==="null")&&(_i+=4,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function NULLFUNC(){return (function(){var _b=_i;return (_s.substr(_i,8)==="nullfunc")&&(_i+=8,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function POINTER(){return (function(){var _b=_i;return (_s.substr(_i,7)==="pointer")&&(_i+=7,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function READONLY(){return (function(){var _b=_i;return (_s.substr(_i,8)==="readonly")&&(_i+=8,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function RETURN(){return (function(){var _b=_i;return (_s.substr(_i,6)==="return")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function RETURNS(){return (function(){var _b=_i;return (_s.substr(_i,7)==="returns")&&(_i+=7,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function SWITCH(){return (function(){var _b=_i;return (_s.substr(_i,6)==="switch")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function SIZEOF(){return (function(){var _b=_i;return (_s.substr(_i,6)==="sizeof")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function STRUCT(){return (function(){var _b=_i;return (_s.substr(_i,6)==="struct")&&(_i+=6,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function TAIL(){return (function(){var _b=_i;return (_s.substr(_i,4)==="tail")&&(_i+=4,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function TEMPORARY(){return (function(){var _b=_i;return (_s.substr(_i,9)==="temporary")&&(_i+=9,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function TO(){return (function(){var _b=_i;return (_s.substr(_i,2)==="to")&&(_i+=2,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function WHILE(){return (function(){var _b=_i;return (_s.substr(_i,5)==="while")&&(_i+=5,true)&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function BITWISE_OP(){return (function(){var _b=_i;return (function(){var _m=_i;return (function(){var _b=_i;return (_s.substr(_i,2)==="<<")&&(_i+=2,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s.substr(_i,3)===">>>")&&(_i+=3,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s.substr(_i,2)===">>")&&(_i+=2,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="|")&&(++_i,true)&&(function(){var _l=_i,_lv=_val,_x=(_s[_i]==="|")&&(++_i,true);_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="&")&&(++_i,true)&&(function(){var _l=_i,_lv=_val,_x=(_s[_i]==="&")&&(++_i,true);_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="^")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(_val=_s.slice(_m,_i),true)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function ADDSUB_OP(){return (function(){var _b=_i;return (function(){var _m=_i;return (function(){var _b=_i;return (_s[_i]==="+")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="-")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(_val=_s.slice(_m,_i),true)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function MULDIV_OP(){return (function(){var _b=_i;return (function(){var _m=_i;return (function(){var _b=_i;return (_s[_i]==="*")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="/")&&(++_i,true)&&(function(){var _l=_i,_lv=_val,_x=(_s[_i]==="/")&&(++_i,true);_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="%")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(_val=_s.slice(_m,_i),true)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function COMP_OP(){return (function(){var _b=_i;return (function(){var _m=_i;return (function(){var _b=_i;return (_s.substr(_i,2)==="<=")&&(_i+=2,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s.substr(_i,2)===">=")&&(_i+=2,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s.substr(_i,2)==="!=")&&(_i+=2,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s.substr(_i,2)==="==")&&(_i+=2,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="<")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]===">")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(_val=_s.slice(_m,_i),true)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function BASE_TYPE(){return (function(){var _b=_i;return INT()&&(function(){ _val = 'int'; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||FLOAT()&&(function(){ _val = 'float'; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||FUNCPTR()&&(function(){ _val = 'funcptr'; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)||POINTER()&&(function(){ _val = 'pointer'; ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function BUILT_IN(){return (function(){var _b=_i;return (function(){var _m=_i;return (function(){var _b=_i;return (_s.substr(_i,3)==="abs")&&(_i+=3,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s.substr(_i,5)==="floor")&&(_i+=5,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s.substr(_i,4)==="itof")&&(_i+=4,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s.substr(_i,4)==="ftoi")&&(_i+=4,true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(_val=_s.slice(_m,_i),true)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function PREFIX_OP(){return (function(){var _b=_i;return (function(){var _m=_i;return (function(){var _b=_i;return (_s[_i]==="-")&&(++_i,true)&&(function(){var _l=_i,_lv=_val,_x=(function(){var _b=_i;return (_s[_i]==="'")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||DIGIT()||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="-")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)})();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="~")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="&")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||(_s[_i]==="*")&&(++_i,true)||(_im=(_i>_im?_i:_im),_i=_b,false)||BUILT_IN()&&(function(){var _l=_i,_lv=_val,_x=SYMBOL_CHAR();_i=_l;_val=_lv;return !_x})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(_val=_s.slice(_m,_i),true)})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};

	...
	: undefined;
{
    ...
    var axisOob = [];                                    /// per-axis bounds findings, handed from foldAxes to subscriptStruct
    var switchStack = [];
    var noForward = false;
    ...
    endStruct = function (name) {
        ...
        /* EVERY borrowed operand the layout block just read, not only the total: a shape holds one scratch
           per non-literal AXIS as well, and those stay live until the `.d.` DEFi lines above quote them. */
        var held = {};
        for (var i = 0; i < s.fields.length; ++i) {
            var f = s.fields[i];
            var parts = (f.dims === undefined ? [ f.size ] : [ f.size ].concat(f.dims));
            for (var j = 0; j < parts.length; ++j) {
                var op = '' + parts[j];
                if (op.charAt(0) !== '<') continue;
                assert(!held[op], 'struct ' + name + ' reuses extent scratch ' + op);
                held[op] = true;
                returnBack(op);
            }
        }
    ...
    declEntry = function (type, elem, struct, words, name, size, at) {
        return { type: type, elem: elem, struct: struct, words: words, name: name, size: size, at: at };
    };
    ...
    mulAddAxis = function (acc, stride, idx, sourceCode, sourceOffset) {
        ...
        var live = (indexKind(acc) === 'runtime' || indexKind(idx) === 'runtime');
        var dst = borrow(live ? '%' : '<');
    ...
    checkAxis = function (slot, axis, idxRV, sourceCode, sourceOffset) {
        ...
        /* Held on the parser because `foldAxes` runs before `subscriptStruct`, which assigns `oobIndex`
           last. Every axis keeps its own finding - see checkIndexUse for why they are not arbitrated. */
        if (oob !== undefined) {
            axisOob.push(oob);
        }
    ...
    foldAxes = function (slot, first, extra, sourceCode, sourceOffset) {
        ...
        if (dims === undefined || dims.length !== extra.length + 1) {
            fail('This array has ' + (dims === undefined ? 'one axis' : dims.length + ' axes')
                    + ', but the subscript names ' + (extra.length + 1),
    ...
    subscriptStruct = function (x, idx, sourceCode, sourceOffset) {
        ...
        var found = axisOob;                             /* the axes this subscript just folded */
        axisOob = [];
        if (oobIndex !== undefined) found.push(oobIndex);
        x.oobIndex = (found.length === 0 ? undefined : found);    /* AFTER the terminal call on every path -
        ...
        if (field.type === 'A') {                                 /* array field -> a fold-able array place for the next [k]
            ...
            x.extent = field.extent || (field.extent =             /* built once per FIELD, not per access */
                    { n: field.size, what: structName + '.' + fieldName,
                      ...
                      /* A SHAPE carries its axes as SYMBOLS, innermost first: a subscript strides by one
                         and a bounds check tests against one, and both need a name rather than a value
                         because the host may own it. Undefined for a 1-D field, which has no shape. */
                      dims: (field.dims === undefined ? undefined
...
function ArgsDecl($){var $v=createParserContext();return (function(){var _b=_i;return (function(){ $._ = []; $.n = 0; ; return true})()&&((function(){var _b=_i;return VarDecl($v)&&(function(){ $._[$.n++] = declEntry($v.type, $v.elem, $v.struct, $v.words, $v.name, $v.size, $v.at); ; return true})()&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_($)&&VarDecl($v)&&(function(){ $._[$.n++] = declEntry($v.type, $v.elem, $v.struct, $v.words, $v.name, $v.size, $v.at); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
function LocalsDecl($){var $v=createParserContext();return (function(){var _b=_i;return (function(){ $._ = []; $.n = 0; ; return true})()&&((function(){var _b=_i;return (function(){var _b=_i;return VarDecl($v)||(_im=(_i>_im?_i:_im),_i=_b,false)||ArrayDecl($v)||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(function(){ $._[$.n++] = declEntry($v.type, $v.elem, $v.struct, $v.words, $v.name, $v.size, $v.at); ; return true})()&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_($)&&(function(){var _b=_i;return VarDecl($v)||(_im=(_i>_im?_i:_im),_i=_b,false)||ArrayDecl($v)||(_im=(_i>_im?_i:_im),_i=_b,false)})()&&(function(){ $._[$.n++] = declEntry($v.type, $v.elem, $v.struct, $v.words, $v.name, $v.size, $v.at); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})(),true)||(_im=(_i>_im?_i:_im),_i=_b,false)})()};
...
function Subscript($){var $idxAt,$subExtra,$s=createParserContext(),$x=createParserContext();return (function(){var _b=_i;return (_s[_i]==="[")&&(++_i,true)&&_($)&&(function(){ $idxAt = _i; $subExtra = []; ; return true})()&&Expr($s)&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_($)&&Expr($x)&&(function(){ if (!dry) $subExtra.push(makeRValue(metaSlot($x._))); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(_s[_i]==="]")&&(++_i,true)&&_($)&&(function(){ if (!dry) { var sb = metaSlot($._); /* ONE index takes the path it took before this rule learned to count - `$s._` is handed on untouched, so a 1-D subscript is byte-identical. */ var sIdx = ($subExtra.length === 0 ? $s._ : foldAxes(sb, $s._, $subExtra, _s, $idxAt)); if ((sb.place && sb.arrayOf) || (sb.type === 'p' && isStructAtom(sb.elem))) subscriptStruct($._, sIdx, _s, $idxAt); else binaryOp('=[]', $._, sIdx, _s, $idxAt); } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};

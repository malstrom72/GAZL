	...
	: undefined;
{
    ...
    var initElemFunc = undefined;                        /// declared funcptr TYPE an `InitList`'s entries must match; the head makeConstant cannot take
    var switchStack = [];
    var noForward = false;
    ...
    /* An INITIALIZER states exactly what an assignment states, so it answers to the same check - but it
       reached the data rows through makeConstant, which knows a type and not a funcptr TYPE, and so
       `global TickFn onTick = wrong` stored a function of any shape under a name that promises one
       shape. Silent: no diagnostic, and the signature row published `: TickFn` regardless. Four doors
       share this because a fix at one of them is the shape of bug that comes back at the next. */
    checkInitFuncPtr = function (elem, x, sourceCode, sourceOffset) {
        if (elem === undefined || !isFuncTypeAtom(elem)) {
            return;
        }
        var m = metaSlot(x);
        checkFuncPtrTarget(elem, bareOperand(m), m.type, m.elem, '', sourceCode, sourceOffset);
    };

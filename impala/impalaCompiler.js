	...
	: undefined;
{
    ...
    function signatureCategoryForDesc(desc) {                     /// 'p:i'->"int-ptr", 'p:Filter'->"Filter-ptr"
        ...
        if (head === 'F') return (tail !== undefined ? tail : 'funcptr');
     ...
     *      F<name>      funcptr type, ends the chain:  FTickFn   *
     *  A tail after `p` is therefore always another DESCRIPTOR,  *
     *  and a tail after `S`/`F` is always a NAME. That is the    *
    ...
    descTypeElem = function (desc) {                     /// 'pi' -> {p,i}; 'FTickFn' -> {F,TickFn}
        return { type: descHead(desc), elem: descTail(desc) };
    };

    structDesc = function (name) { return 'S' + name; };
    funcTypeDesc = function (name) { return 'F' + name; };
    ...
    isFuncTypeAtom = function (desc) {                   /// does descriptor `desc` denote a funcptr type?
        return desc !== undefined && desc.charAt(0) === 'F' && desc.length > 1;
    };

    /* The NAME inside an `S`/`F` descriptor. Takes a descriptor strictly - it is NOT identity on a bare
       name, because a struct genuinely called `Fn` would strip to `n`. */
    descName = function (desc) {
        assert(isStructAtom(desc) || isFuncTypeAtom(desc), 'descName needs an S/F descriptor');
    ...
    subscriptStruct = function (x, idx, sourceCode, sourceOffset) {
        ...
        if (idxKind !== 'runtime') {                             /* an assemble-time index -> fold into the compile-time offset parts */
            ...
            if (elemStruct) setPlace(x, x.baseKind, x.base, x.offParts, elemName, x.root, undefined, x.dynIndex);
            ...
            if (elemStruct) {
                ...
                returnBack(idxRV);
                returnBack(arrPtr);
                setPlace(x, 'pointer', elemPtr, [], elemName, elem);
    ...
    fieldAccess = function (x, fieldName, arrow, sourceCode, sourceOffset) {
        ...
        if (x.place) {
            ...
            bk = 'pointer'; base = makeRValue(x); offParts = []; structName = descName(x.elem); root = x.elem;
        } else {
    ...
    checkPtrAssign = function (leftx, rightx, sourceCode, sourceOffset) {
        ...
        if (leftx.type === 'F' && isFuncTypeName(leftx.elem)) {   /* named funcptr type target */
    ...
    unaryOp = function (operator, expr, sourceCode, sourceOffset) {
        ...
        if (operator === '*' && expr.type === 'p' && isStructAtom(expr.elem)) {
            setPlace(expr, 'pointer', makeRValue(expr), [], descName(expr.elem), expr.elem);
            return;
        }
        ...
        if (operator === '&' && expr.place) {
            ...
            setElem(expr, structDesc(structName));
            return;
        }
        ...
        if (key === '=*') {
            ...
            /* &x yields a pointer to x's type - but `?` (an element of an Impala 1 untyped
               ...
               its own: `F` + its name IS the descriptor. */
    ...
    lookup = function (x, name, isGlobal, sourceCode, sourceOffset) {
        ...
        if (!isGlobal && (p = sym.locals['$' + name])) {
            ...
            if (p.type === 'S') {                                 /* struct value local -> a place */
                setPlace(x, 'local', '$' + name, [], descName(p.elem), p.elem);
                return;
            }
        ...
        if (isGlobal && (p = sym.globals[name])) {
            ...
            if (p.type === 'S') {                                 /* struct value global -> a place in global memory */
                setPlace(x, 'globalAddr', '&' + name, [], descName(p.elem), p.elem);

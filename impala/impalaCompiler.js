	...
	: undefined;
{
    ...
    function signatureCategoryForDesc(desc) {
        if (desc === undefined) {
            return 'unknown';
        }
        var text = signatureParamCategory(desc[desc.length - 1]);
        for (var wi = desc.length - 3; wi >= 0; wi -= 2) {
            text += '-ptr';
        }
        return text;
    }

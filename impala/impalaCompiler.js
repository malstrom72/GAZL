	...
	: undefined;
{
    ...
    /* A CONSTANT index against a KNOWN extent. One rule for every array, called from both subscript
       ...
       NOTHING IS REPORTED HERE, because whether this is an error depends on what happens next, and the
       rule is `docs/Impala2Review.md`'s: a DEREFERENCE with an out-of-range constant index is a
       ...
       is also why Impala needs no one-past-the-end carve-out where C does. */
    checkConstIndex = function (extent, idxRV, sourceCode, sourceOffset) {
        if (extent === undefined || !/^[0-9]+$/.test('' + extent.n)
                || !/^#-?[0-9]+$/.test(idxRV)) {
            return undefined;
        }
        var ck = parseInt(idxRV.substr(1), 10), cn = parseInt(extent.n, 10);
        return (ck < 0 || ck >= cn) ? { k: ck, n: cn, src: sourceCode, off: sourceOffset } : undefined;
    };
    ...
    arrayExtent = function (sym, name) {
        return (sym === undefined || sym.extentSym === undefined) ? undefined
                : { n: sym.extentNum, sym: sym.extentSym, what: name };
    };
    ...
    /* An out-of-range constant index survived the subscript as a flag because only its USE decides
       whether it is an error. Anything that reads or writes the element lands here; `&` cleared it. */
    ...
    setPlace = function (rec, baseKind, base, offParts, structName, root, arrayOf, dynIndex, extent) {
        ...
        slot.oobIndex  = undefined;                                /* pooled slot: never inherit another subscript's finding */
    };
    ...
    subscriptStruct = function (x, idx, sourceCode, sourceOffset) {
        ...
        if (/^#[0-9]+$/.test(idxRV)) {                           /* constant index -> fold into the compile-time offset parts */
            ...
            metaSlot(x).oobIndex = oobIndex;                       /* set AFTER the terminal call - both clear the slot */
            return;
        }
        ...
        if (x.baseKind === 'local' && x.dynIndex === undefined) { /* a frame place with a single runtime index: keep it
            ...
            metaSlot(x).oobIndex = oobIndex;                     /* a NEGATIVE constant reaches this path:
                                                                    the folding test above has no minus sign */
            return;
        }
    ...
    binaryOp = function (operator, leftx, rightx,
        ...
        if (operator === '=[]') {
            ...
            if (leftx.operator === ':=' && op1[0] === '&') {
                ...
                xOob = checkConstIndex(xt, op2, sourceCode, sourceOffset);
                ...
                xOob = checkConstIndex(xt, op2b, sourceCode, sourceOffset);

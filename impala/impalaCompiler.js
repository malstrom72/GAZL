	...
	: undefined;
{
    ...
    formatStructSignatureComment = function (name, sourceName, sourceCode, sourceOffset) {
        return structSignatureRow(name, false, sourceName, sourceCode, sourceOffset);
    };

    formatExternStructSignatureComment = function (name, sourceName, sourceCode, sourceOffset) {
    ...
    /* Flattened dotted access-paths of a struct (memoized): every path that reaches a field,
       recursing through nested by-value structs. Biquad -> ['b0','z1']; Voice ->
       ['note','lo','lo.b0','lo.z1','gain']. Used to emit a single offset constant per reachable
       nested path so field access is one symbolic operand (no runtime address math). */
    structPaths = function (name) {
        var s = structs[name];
        if (!s) return [];
        if (s.pathsCache) return s.pathsCache;
        var paths = [];
        for (var i = 0; i < s.fields.length; ++i) {
            var f = s.fields[i];
            paths.push(f.name);
            if (f.type === 'S') {
                ...
                for (var j = 0; j < inner.length; ++j) paths.push(f.name + '.' + inner[j]);
            }
        }
        s.pathsCache = paths;
        return paths;
...
function Destructure($){return (function(){var _b=_i;return (function(){ if (!dry) destTargets = []; ; return true})()&&DestTarget($)&&((function(){for(var _n=0;(function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_($)&&DestTarget($)||(_im=(_i>_im?_i:_im),_i=_b,false)})();++_n);return _n>0})())&&(_s[_i]==="=")&&(++_i,true)&&_($)&&(function(){   /* PARKED for Impala 3.0 - see docs/ParkedFeatures.md */ fail('Destructuring assignment is not supported in Impala 2.0', _s, _i, 'E429', 'assign one value per statement'); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};

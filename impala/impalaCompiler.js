	...
	: undefined;
{
    ...
    arraySubscriptFlat = function (x, rvs, sourceCode, sourceOffset) {
        ...
        var dims, elemDesc, ptrBase = false;

        if (x.dims !== undefined && x.dims.length >= 2) {            /* a named multidimensional array */
            dims = x.dims;
            elemDesc = x.elem;
            ...
            /* a shape pointer p:[dims]:elem - index WITHIN (rvs == rank) or WALK one outer axis (== rank+1) */
            dims = arrayAtomDims(descHead(x.elem));
            elemDesc = descTail(x.elem);
            ptrBase = true;
            if (rvs.length !== dims.length && rvs.length !== dims.length + 1) {
                fail('This shape pointer is ' + dims.length + '-dimensional; index it with '
                        + dims.length + ' or ' + (dims.length + 1) + ' subscripts, got ' + rvs.length,
                        sourceCode, sourceOffset, 'E421');
                while (rvs.length < dims.length) rvs.push('#0');
                rvs = rvs.slice(0, dims.length + 1);
            }
        } else {
        ...
        /* flat = rvs[0]; for each later index: flat = flat * mults[k-1] + rvs[k] */
        var acc = rvs[0];
        for (var k = 1; k < rvs.length; ++k) {
            var ik = rvs[k];
            var mk = parseInt('' + mults[k - 1], 10);
        ...
        /* element access at the flat offset */
        if (ptrBase) {                                               /* runtime pointer base -> PEEK/POKE base + flat */
            makeMeta(x, '=[]', tp, null, makeRValue(x), acc);
        } else {

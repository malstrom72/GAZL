    ...
    processBranches = function () {
        ...
        /* BRANCH THREADING and RETURN DUPLICATION (docs/GAZLAssemblerOptimizations.md items 4 and 5).
           ...
           statement, so an early exit is a `goto` to the end label, which is where RETU sits.
              Branch THREADING (item 4) is deliberately NOT here. Adopting a target's target reproduces
           the prototype's `Priyome`: `Symbol not found (in expected scope): .f5` - a label graph is not
           a safe thing to rewrite from inside the compiler that is still minting it, and the assembler
           does the same job over RESOLVED addresses where the question cannot arise. Left to it. */
        var leadsTo = {};
        for (i = 0; i < metacode.length; ++i) {
            if (metacode[i].operator !== '<--') continue;

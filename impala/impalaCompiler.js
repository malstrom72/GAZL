	...
	: undefined;
{
    ...
    var tailCtx        = undefined;                   /// { name, label, rec, used } of the function being parsed - `tail` needs its entry label
...
function Tail($){var $tailAt,$id=createParserContext();return (function(){var _b=_i;return TAIL($)&&_($)&&(function(){ $tailAt = _i; ; return true})()&&Identifier($id)&&(function(){ if (_s.charAt(_i) !== '(') return false;   /* pure lookahead: nothing to undo */ if (inlineMode) { fail('tail cannot be used in an inline function', _s, $tailAt, 'E468', 'an inline body expands into its caller and runs in that frame, so there is no frame of its own to reuse'); } if (!gazl2) { fail('tail requires --gazl2', _s, $tailAt, 'E466', 'the parameter rewrite it compiles to only assembles inside a GAZL #2 region'); } if ($id._ !== tailCtx.name) { fail('tail can only target the enclosing function in Impala 2.0 (' + $id._ + ' is not ' + tailCtx.name + ')', _s, $tailAt, 'E467', 'a general tail call needs a GAZL TAIL instruction, which GAZL 2 does not have - call it, or restructure into self-recursion'); } lookup($._, $id._, false, _s, $tailAt); metaSlot($._).tailCall = true; ; return true})()&&FuncCall($)&&(_s[_i]===";")&&(++_i,true)&&_($)&&(function(){ releaseMeta($._); ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};

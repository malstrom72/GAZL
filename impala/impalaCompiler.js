	...
	: undefined;
{
    ...
    /* fp->field  /  place.field  (Step 2 slice 1: scalar fields through a struct pointer) */
    fieldAccess = function (x, fieldName, arrow, sourceCode, sourceOffset) {
        x = metaSlot(x);

        var structName;
        if (x.type === 'p' && isStructAtom(x.elem)) {
            if (!arrow) {
        ...
        if (field.type === 'S' || field.type === 'A') {
            fail('Field ' + fieldName + ' of ' + structName
                    + ' is a nested struct/array; that access is not yet supported',
                    sourceCode, sourceOffset, 'E418');
        }

        /* lower as  x[field.offset]  then stamp the field's own type */
        var idx = {};
        makeMeta(idx, ':=', 'i', undefined, '#' + field.offset, undefined);
        binaryOp('=[]', x, idx, sourceCode, sourceOffset);
        x.type = field.type;
        x.elem = field.elem;
    };

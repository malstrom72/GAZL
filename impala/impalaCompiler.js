	...
	: undefined;
{
    ...
    arraySubscriptFlat = function (x, rvs, sourceCode, sourceOffset) {
        ...
        if (dims === undefined || dims.length < 2) {
            fail('A comma-separated index list needs a multidimensional array',
                    sourceCode, sourceOffset, 'E421');
            return;
        }
        if (rvs.length !== dims.length) {
            fail('This array has ' + dims.length + ' dimensions but '
                    + rvs.length + (rvs.length === 1 ? ' index was given' : ' indices were given')
                    + ' (write every axis)', sourceCode, sourceOffset, 'E421');
            while (rvs.length < dims.length) rvs.push('#0');          /* recover to keep parsing */
        }
        var lelem = x.elem;
        var tp    = descHead(lelem);                        /* scalar element type ('i' / 'f') */

        /* flat = i0; flat = flat * dims[j] + ij   (row-major) */
        var acc = rvs[0];
        for (var j = 1; j < dims.length; ++j) {
            var ij = rvs[j];
            var dj = parseInt('' + dims[j], 10);
            if (acc.charAt(0) === '#' && ij.charAt(0) === '#') {
                acc = '#' + (parseInt(acc.substr(1), 10) * dj + parseInt(ij.substr(1), 10));
            } else {
                var t = borrow('%');
                emit('*', 'i', t, acc, '#' + dj);
                emit('+', 'i', t, t, ij);
                if (acc.charAt(0) === '%') returnBack(acc);
                if (ij.charAt(0)  === '%') returnBack(ij);
                acc = t;
            }
        }

        /* element access at the flat offset (mirrors binaryOp '=[]': fold a constant, else PEEK/GETL) */
        var op1 = x.operands[1];
        ...
        x.type = descHead(lelem);
        x.elem = descTail(lelem);
...
function Subscript($){var $s=createParserContext(),$subNode,$subMulti,$subRV,$sc=createParserContext();return (function(){var _b=_i;return (_s[_i]==="[")&&(++_i,true)&&_($)&&Expr($s)&&(function(){ if (!dry) { $subNode = $s._;      /* single-index path keeps the node */ /* multi-dim only: capture each index value NOW, while its meta is fresh */ $subMulti = (metaSlot($._).dims !== undefined && metaSlot($._).dims.length >= 2); $subRV = $subMulti ? [ makeRValue(metaSlot($s._)) ] : null; } ; return true})()&&((function(){while((function(){var _b=_i;return (_s[_i]===",")&&(++_i,true)&&_($)&&Expr($sc)&&(function(){ if (!dry) { if (!$subMulti) { $subMulti = true; $subRV = [ makeRValue(metaSlot($subNode)) ]; } $subRV.push(makeRValue(metaSlot($sc._))); } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})());})(),true)&&(_s[_i]==="]")&&(++_i,true)&&_($)&&(function(){ if (!dry) { if ($subMulti) { arraySubscriptFlat($._, $subRV, _s, _i); } else { var sb = metaSlot($._); if (sb.type === 'p' && isStructAtom(sb.elem)) structSubscript($._, $subNode, _s, _i); else binaryOp('=[]', $._, $subNode, _s, _i); } } ; return true})()||(_im=(_i>_im?_i:_im),_i=_b,false)})()};

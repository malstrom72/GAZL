	...
	: undefined;
{
    ...
    copyStructArg = function (argMeta, winSlot, words) {
        ...
        for (var k = 0; k < words; ++k) {                         /* reserve the window slots (count is compile-time) */
            claimSlot(winSlot + k);
        }
        var sz = structAllocSize(argMeta.struct);           /* symbolic *.z.Struct - never a baked word count */
        var dst = borrow('%');
        emit('=&', 'p', dst, '%' + winSlot, sz);            /* ADRL address of the window region */
        var src = placeAddress(argMeta);                    /* address of the source struct */
        emit('copy', '?', dst, src, sz);                    /* COPY dst src *.z.Struct */
        returnBack(src);
        returnBack(dst);

...
- [x] Maintain and modernize the generated compilers (`impalaCompiler.js`, `jspegCompiler.js`) once grammar updates are defined.
...
- [x] Translate `impala.ppeg` semantics into `impala.jspeg` while preserving runtime behavior.
...
- [x] Diff remaining statement actions against the PPEG grammar to confirm label management and branch folding stay byte-for-byte compatible.
...
- [x] Compare the `DoWhile`, `While`, `Loop`, and `For` rules to ensure borrowed temporaries, loop labels, and `returnBack` usage mirror the `impala.ppeg` lifecycle, keeping `$$parser.processBranches` fed with identical meta-instruction streams.
                        ...
                        - Added a `control.impala` parity fixture exercising do/while/loop/for scaffolding so both toolchains prove they emit identical branch skeletons across nested control flow and temporary recycling.【F:impala/jspeg/testdata/control.impala†L1-L44】【F:impala/jspeg/testdata/control.pika.gazl†L1-L37】【F:impala/jspeg/jspegCompilerTests.js†L118-L124】
                - [x] Exercise `Copy` and `Switch` (including `CaseExpr`) through both toolchains to verify `makeConstant`/`dropHash` folding, case-comment emission, and default-label wiring remain in sync.
                        ...
                        - The new control-flow fixture also drives `copy` and multi-`case` switches so the JSPEG regression suite diff-checks those code paths against the legacy compiler.【F:impala/jspeg/testdata/control.impala†L1-L44】【F:impala/jspeg/testdata/control.pika.gazl†L1-L37】【F:impala/jspeg/jspegCompilerTests.js†L118-L124】
        ...
        - [x] Break down the remaining feature work blocking advanced fixtures so each gap in `impala.jspeg` is documented alongside its runtime touch points.
                - The regression harness now enumerates every legacy Impala fixture under `tests/impala/sources`, recompiles them with the JSPEG compiler, and diff-checks the latin-1 output against the checked-in goldens, proving the JavaScript port matches the PPEG emitter across the full program suite.【F:impala/jspeg/jspegCompilerTests.js†L117-L164】【F:impala/jspeg/jspegCompilerTests.js†L217-L260】
                ...
                - [x] Local arrays and compound locals: extend `LocalsDecl`/`ArgsDecl` to recognise `array` declarators, plumb the resulting pointer/length bookkeeping into `makeLValue`, and ensure `InitList` materialises the same `TEMP`/`DATA` runs used for global arrays so fixtures relying on stack arrays (e.g., `inputTest.impala`) compile.【F:impala/jspeg/impala.jspeg†L1571-L1599】【F:impala/jspeg/impala.jspeg†L2001-L2034】【F:impala/jspeg/jspegCompilerTests.js†L131-L170】
                        - `LocalsDecl` now clones array declarators alongside scalars, `Copy` borrows and releases the hashed-length meta before emitting the instruction, and the `inputTest` parity fixture confirms stack-allocated buffers compile identically to the PPEG baseline.【F:impala/jspeg/impala.jspeg†L1571-L1599】【F:impala/jspeg/impala.jspeg†L2001-L2034】【F:impala/jspeg/jspegCompilerTests.js†L131-L170】
                - [x] Constant tables and compile-time folding: guard `makeRValue` and `mulDivOp` against partially populated meta operands so `makeConstant` can fold string/function pointer arrays, and keep `InitList` chunking aligned with the PPEG emitter; parity now covers the legacy `calc` golden with raw tabs.【F:impala/jspeg/impala.jspeg†L481-L515】【F:impala/jspeg/impala.jspeg†L649-L699】【F:impala/jspeg/impala.jspeg†L1624-L1661】【F:impala/jspeg/jspegCompilerTests.js†L188-L205】
                        - Added the `BitMaskMod_code` parity run so deferred string pooling and multi-line `DATA` blocks stay byte-for-byte aligned with the PPEG emitter before enabling further string optimisations.【F:impala/jspeg/jspegCompilerTests.js†L207-L216】【F:tests/impala/golden/BitMaskMod_code.gazl†L1-L82】
                        - Normalised string pooling to 8-bit code points before deduplication so unicode escapes and latin-1 assets mirror the legacy emitter, unblocking the `ImpalaDemo` and `buffer` parity fixtures.【F:impala/jspeg/impala.jspeg†L78-L99】【F:impala/jspeg/impala.jspeg†L121-L140】【F:impala/jspeg/jspegCompilerTests.js†L205-L225】
                        - Registered the `ImpalaDemo` and `buffer` regressions under the legacy harness to cover mixed control-flow, string pooling, and latin-1 screen assets emitted by the original toolchain.【F:impala/jspeg/jspegCompilerTests.js†L205-L232】【F:tests/impala/golden/ImpalaDemo.gazl†L233-L242】【F:tests/impala/golden/buffer.gazl†L490-L498】
                - [x] Native bridge coverage: replicate the `extern native` scaffolding by mapping `native` function declarations through `declare`, translating parameter passing through `borrowForCall`, and threading the host I/O helpers referenced by `inputTest.impala` so runtime glue matches the legacy compiler.【F:impala/jspeg/impala.jspeg†L1445-L1490】【F:impala/jspeg/impala.jspeg†L1660-L1695】【F:impala/jspeg/jspegCompilerTests.js†L131-L170】
                        - `ExternDecl` routes `extern native` functions through `declare`, the call helpers recycle argument windows after `borrowForCall`, and the regression harness' `inputTest` comparison exercises the host-facing bridge end-to-end.【F:impala/jspeg/impala.jspeg†L1445-L1490】【F:impala/jspeg/impala.jspeg†L1660-L1695】【F:impala/jspeg/jspegCompilerTests.js†L131-L170】
                - [x] `_` sentinel initialisation: align the parser bootstrap so the root accumulator preallocates a meta record instead of `{_: void}`, mirroring the PPEG behaviour and keeping `assign()` supplied with a well-formed left-hand slot without relying on the JS harness guard.【F:impala/jspeg/impalaJsCompilerRunner.js†L73-L90】【F:impala/jspeg/impala.jspeg†L429-L447】【F:impala/impala.ppeg†L113-L172】
                - [x] Meta clean-up sequencing: ensure `$$parser.end` flushes deferred strings, resets counters, and clears borrowed temporaries when compilation aborts mid-rule so subsequent runs do not inherit stale meta instructions or leaked `%` tokens.【F:impala/jspeg/impala.jspeg†L1283-L1351】【F:impala/jspeg/impala.jspeg†L209-L295】【F:impala/jspeg/jspegCompilerTests.js†L171-L205】
                        - Added a regression that intentionally triggers a `copy` type error before recompiling the `smoke` fixture, verifying that deferred strings, branch queues, and transient stocks reset between runs and that post-abort compilations still match the baseline output.【F:impala/jspeg/jspegCompilerTests.js†L171-L205】
- [x] Establish regression comparisons between the PPEG-driven and JSPEG-driven Impala compilers.
        - Added the `switchtest` parity run against the legacy `tests/impala` golden output so the JSPEG harness now exercises
          a full-size control-flow fixture sourced from the PPEG toolchain, guarding register reuse and switch lowering against
          regressions before promoting additional test cases.
...
- [x] Fix the `_`-slot initialisation bug uncovered while running the JS harness: the root parse object now seeds a meta record, so `assign()` sees the expected operator and no longer triggers the meta-missing guard.【F:impala/jspeg/impalaJsCompilerRunner.js†L73-L90】【F:impala/jspeg/impala.jspeg†L704-L787】

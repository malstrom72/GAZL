# Impala (JS)

This is the JavaScript version of the Impala compiler, generated by the JSPEG grammar `impala.jspeg`. It replaces the PikaScript-based `impala.pika` for Node.js workflows and CI, while keeping output identical to the legacy toolchain.

## Artifacts

All files live under `impala/jspeg/`:

- `impala.jspeg` – Impala grammar with JavaScript actions.
- `impalaCompiler.js` – generated Impala compiler (CommonJS export) produced by JSPEG.
- `impala.node.js` – simple Node CLI for compiling/running Impala programs using the JS compiler.
- `impalaJsCompilerRunner.js` – programmatic adapter that runs `impalaCompiler.js` in a sandboxed VM and formats output.
- `runJspegTests.js` – Impala parity runner (compares compiled output to golden `.gazl`).
- `testdata/` – small fixtures for parity checks.

For JSPEG internals, self-hosting, and regeneration, see `JSPEG.md`.

## Quickstart (CLI)

Compile to stdout or to a file:

```bash
# To stdout
node impala/jspeg/impala.node.js compile path/to/source.impala - 0x4d2

# To a file
node impala/jspeg/impala.node.js compile path/to/source.impala path/to/output.gazl 42

# From stdin to stdout
cat path/to/source.impala | node impala/jspeg/impala.node.js compile
```

Run a compiled program through the GAZL VM (requires `output/GAZLCmd`, built via `build.sh`):

```bash
node impala/jspeg/impala.node.js run path/to/source.impala
```

Notes:
- I/O uses Latin‑1 to match existing fixtures.
- With no input path, the CLI reads from stdin. With output `-` or omitted, it writes to stdout.
- Output formatting matches the PikaScript CLI (alignment tabs retabulated to a standard 4-space grid, trailing newline by default).

## Programmatic API

Use `compileWithJsImpala(source, options)` from `impalaJsCompilerRunner.js` for programmatic compilation with deterministic output:

```js
const { compileWithJsImpala } = require('./impala/jspeg/impalaJsCompilerRunner');

const gazl = compileWithJsImpala(sourceText, {
        randomId: 0x4d2,      // seed for deterministic IDs
        retabulate: true,     // expand alignment tabs to 4-space tab stops
        trailingNewline: true // add trailing newline (default when retabulate === true)
});
```

Details:
- Runs the generated compiler in a Node `vm` context so required globals (like `output`) stay sandboxed. The compiler no longer creates sloppy globals.
- Parser actions now build real meta-records directly; helper calls receive the holder object (`$$.`) so no meta-slot shim is required.
- `retabulate: false` preserves the original alignment tabs for byte-for-byte comparisons.

 

## Tests (Impala parity)

Validate that the JS compiler matches the recorded outputs and legacy fixtures:

```bash
node impala/jspeg/runJspegTests.js                # compare sources to golden .gazl
node impala/jspeg/runJspegTests.js --makegold     # regenerate golden outputs
```

For JSPEG self-hosting and regeneration tests, see `JSPEG.md`.

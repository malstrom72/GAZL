...
grammar `impala.jspeg`. It replaces the PikaScript-based `impala.pika` for the
active command-line workflow, while keeping output identical to the legacy
toolchain.
...
- Output formatting matches the PikaScript CLI: alignment tabs are retabulated
...
```bash
./output/NuXJS -s output/impala.nuxjs.js source.impala 0x4d2 source.impala > output.gazl
```
...
`impala/jspeg`; the build stages both files together in `output/`.

...
function main() {
	...
	try {
		...
		const importFile = writeFixture(tempDir, 'moduleB.gazl', [
			'; signatures version=1',
		...
		assertCondition(/Call to foo does not match its definition/.test(failure.stderr), 'validator did not report expected mismatch error');

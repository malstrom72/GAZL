...
side of the repo: a blocker list in `docs/ParkedFeatures.md` stayed stale for a week and sent a research
pass down a dead end, because a fact had two homes and only one was updated. See the rule at the bottom of
`docs/README.md` on `Impala2`: one fact, one home, link don't copy.

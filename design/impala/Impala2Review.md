...
3. **No multidimensional arrays.** You hand-stride flat arrays (`grid[y * W + x]`), and the stride becomes a
   ...
   the stride.
4. **The diagnostics backlog** (`design/impala/CompileTimeHardening.md`) — five shapes the compiler accepts that the
...
`design/ParkedFeatures.md` does **not** record the decay decision — it lives only in
...
`docs/impala/Impala2.md:159-164` promises the opposite, and `docs/impala/Impala.md:451` states `p[i]` is equivalent to
...
- **Locals are not zero-filled, and `docs/impala/Impala2.md:430` says they are** ("Uninitialized struct storage is
...
- **`extern struct` host layout is essentially unchecked**: overlapping offsets, offsets past `.z.Name`, and
  ...
  `design/impala/ExternPrototypes.md:26-28` claims gazl-validate catches drift; it is name-presence-only and does not
  run at all in the motivating use case. `design/impala/StructLayoutConstants.md:227-229` correctly calls it deferred.
...
- **Typed-array initializers are not element-checked.** `global int array T[4] = { 1, 2.0, 3, 4 }` compiles,
  contradicting `docs/impala/Impala2.md:260` which states it is an error. `InitList` (`:4042-4079`) uses each
...
- **`switch (x == lo to hi)`: `hi` is exclusive**, and `docs/impala/Impala.md:328` says inclusive.
...
- **Three signature grammars.** `function` and `extern function` require parameter names *and* a return name
  ...
  `docs/impala/Impala2.md:659` claims they mirror each other — **[V]** false.
...
- **`docs/impala/Impala.md:261` says forward references work.** They do not — but the E403 note is excellent, and
  even finds the later definition.
- **`docs/impala/Impala.md:316-324`'s documented `goto`-out-of-loop idiom does not compile** — a trailing label
  needs `finished: ;`. It is the only documented way to break out of a loop.
- **`docs/impala/Impala.md:28-30`'s reserved-word list is missing all six new keywords**, so it gives a false
...
- `design/impala/StructLayoutConstants.md:220-226` lists **E418, E424, E425** as implemented extern-struct guards.
...
- `(funcptr array) table` casts (`docs/impala/Impala2.md:218`) do not parse — the cast grammar has no `array`.
...
- **The E-code registry in `docs/impala/Impala2.md:1136-1160` is ~29 codes behind** (E410-E442 are all
...
- **The legacy manual-concatenation struct model no longer links** **[V]**. Two byte-identical `struct`
  ...
  `docs/impala/Impala2.md:579-632` is dead text; the working pattern (one unit `struct`, the rest
  `extern struct` with a body) is documented nowhere.
- `design/impala/StructLayoutConstants.md:198-209` *understates* the design: it claims nested/array field access
...
`Code size: 0 ... Could not locate function: main`. `docs/impala/UsageExample.md` has the correct 4-argument form.
...
files. `docs/impala/Impala.md:530+` describes gazl-validate at length without ever saying GAZLCmd is the real one.
...
multidim-array work (`design/ParkedFeatures.md`) lands, since a 2-D subscript scales on both axes.
